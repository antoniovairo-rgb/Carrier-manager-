...
| **AI Vision Review Engine** | 🟡 scaffold (opt-in) | `lib/ai-vision.mjs`: pipeline che analizza un campione di screenshot con un modello vision → recognition + score (Readability/Cinematic/Motion/Football-IQ) + verdict, in `out/validate/ai-vision.json` + Dashboard. **Non blocca la CI** (secondo livello). Provider-agnostic (default Anthropic), si **attiva con `CPM_VISION_API_KEY`**; senza chiave skip pulito. Manca: cattura video/sequenza, Situation Recognition formale, confronto build/storico |
