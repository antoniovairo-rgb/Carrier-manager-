...
| **Validator Engine** | 🟡 parziale | esistenti (frame-congelato/analitica): Technical, Ball (`computeArc`), Camera (regia per-pattern), Player (movements cap), + Score Aggregator analitico. **Event/Timeline** ✅ avviati (check `timeline`, LMQP-2): backbone narrativo per-highlight (HighlightForced→ActionResolved, intent coerente). **Semantic** 🟡 avviato: invarianti CINE sulla decisione live (LMQP-3), regressione decisione vs baseline (LMQP-5), **coerenza dell'esito** (LMQP-6: outcome-key taxonomy `goal/assist/save/recovery` vs `miss/intercept/…` + `ok⟺successo`). **Mancano ancora:** Semantic completo per-Situation (timeline a beat fini), Football Intelligence, Cinematic, Motion |
...
| **Timeline Engine** | 🟡 **avviato (LMQP-1, 5.35)** | bus eventi osservabile `MATCH_TL` + probe `window.__CPM_TIMELINE`; registra MatchStart/HighlightForced/ActionResolved/HighlightAdvance. Manca: timeline per-beat fine-granulare (ricezione/tocco/arco) e persistenza/replay |
| **Event Driven** | 🟡 **avviato (LMQP-1, 5.35)** | `cpmEmit(type,payload)` emette eventi ai punti chiave del flusso HL; consumato dal check `timeline` (LMQP-2). Manca: eventi fini (BallTouched/CameraChanged) e un Event Registry formale (cap. 3.13) |
| **Replay Engine** | 🟡 avviato (LMQP-10/11) | `lib/replay-trace.mjs` persiste tracce **per-frame** (camera/palla/giocatori/fase) del post-highlight in `out/validate/replay/sit_NNN.json`; la **Dashboard include un Replay Viewer** 2D top-down che le rigioca (play/scrub, LMQP-11). Mancano ancora: registrazione completa di ogni HL (non solo il campione), eventi per-frame, snapshot/diff tra build, debugger avanzato |
| **Failure Collector** | 🟡 avviato (LMQP-7) | `lib/failure-collector.mjs` produce **`run-summary.json`**: pacchetto compatto machine-readable (esito, per-check pass/issue/warn, `failures[]` normalizzate `{check,gi,msg,situation,intent,shot}`, coverage, baseline drift, performance, **fingerprint deterministico** per dedup/trend). Mancano ancora: video/snapshot per-fail, classificazione/priorità/retention, dipendenza dal Replay Engine |
| **Regression Engine** | 🟡 avviato (LMQP-5) | due baseline committate confrontate dal gate: `golden-sigs.json` (firma stato `{htx,hty,cam,ch,ca}`) + **`decision-baseline.json`** (firma decisione `intent\|hlType\|ballState` per situation, LMQP-5 5.36). Drift = FAIL, rigenerabili. Mancano ancora: baseline ricca (replay/score/metriche), severità, gallery, visual/score comparison, storico |
| **Performance Monitor / Resource Manager** | 🟡 avviato (LMQP-8) | `lib/perf-monitor.mjs` misura load-time (navigation), JS heap (Chromium) e frame-timing (avg/p95/fps) col render-loop attivo → `run-summary.json`/Dashboard. **Warn-only** (FPS headless = software-rendered, soglie su floor headless-realistici, mai FAIL). Mancano ancora: CPU/GPU, leak/zombie detection automatica, stress test multi-worker, grafici storici; health-check processi tuttora **manuale** (`ps`) |
| **Dashboard (Visual QA / LMDP)** | 🟡 avviato (LMQP-9) | `tests/visual/report.mjs` → `out/validate/index.html` ora include la **LMQP Dashboard** dal `run-summary.json`: griglia dei 10 check, KPI (esito/falliti/failure/warning/fingerprint), Performance, Decision baseline drift, Coverage a chip (intents/hlTypes/ballStates/outcomeKeys), tabella failure. Mancano ancora: Home live multi-run, Replay/Timeline Viewer, Visual Comparison, AI Review, search/filtri, Quality Score globale, notifiche |
| **AI Vision Review Engine** | 🟢 framework | `lib/vision/`: `VisionProvider`→`VisionReviewEngine`→`ProviderRegistry` (Open/Closed). Provider: **Ollama** (1°, locale+cloud Bearer), Anthropic, OpenAI/Gemini predisposti. Runner `ai-vision-review.mjs`: cattura Playwright multi-fase → engine (cache/dedup/retry/fallback/Failure-Package) → scoring (Quality/Confidence/Severity/Priority/verdict) → report HTML+JSON. Config via `.env`, **non blocca la CI** (skip se provider giù). 28 test (`test:vision`). Manca: video continuo, Situation Recognition formale, confronto build/storico, analyze() OpenAI/Gemini |
