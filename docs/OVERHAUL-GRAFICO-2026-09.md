...
- 21/09 — **G8.10 · «il tab carriera è lunghissimo» (rilievo PO), e il metro impara a dire DOVE.** Il metro confermava la lunghezza (**3.575 px = 3,91 schermate**, la peggiore del gioco) ma non diceva **quale pezzo** la allungasse: così si accorcia a caso. Aggiunta alla griglia la tabella **«dove stanno i pixel»** — i blocchi di primo livello dello scorritore con altezza, quota e testo, per le sole schermate sopra le due schermate. **Difetto mio dello strumento, trovato e corretto nella stessa corsa**: la discesa si fermava troppo presto e il Profilo rispondeva «un blocco solo da 3.644 px», cioè la pagina intera; ora si scende finché il figlio più alto vale meno dell'80 %. **La diagnosi**: il Profilo è un elenco di **quindici sezioni tutte aperte** — Milestone **661 px (18,5 %)**, Corsa ai Premi 292, Albo d'oro 187, La Stampa 183, Contratto 159, Match del Cuore 125. Sei sezioni di **consultazione** richiuse con la `Fisarmonica` (Attributi e Statistiche restano aperti: si **usano**). **Misurato a 412 px: 3.575 → 2.444 px, 3,91 → 2,67 schermate (−1.131 px)**, e il numero non è comprato nascondendo — rimisurato con **tutte le fisarmoniche aperte**: difetti **0/2.881** nodi di testo, contrasto **0/2.249**.
- 21/09 — **Le ridondanze hanno un censimento** (richiesta PO «verifica anche eventuali ridondanze delle informazioni»). Si contano le **etichette**, non i numeri: un «14» si ripete per caso, un «Assist» no. Due letture con rimedi opposti: la **colonna** (stessa grandezza ripetuta dentro una schermata, da togliere) e la **riga** (stessa grandezza in più tab, che può essere un cruscotto voluto). **Limite dichiarato subito, perché cambia la lettura**: le otto schermate di carriera condividono la **testata dell'eroe**, quindi una riga da 8 (OVR, forma, morale, fatica) è il cruscotto, non un difetto. **Le ridondanze vere, dentro la schermata**: Profilo **«Gol» 8 volte** e «Assist» 4 · Dashboard **«Settimana» 7** e «Gol» 3 · Nazionale «Stagione» 5 · Agente «Stipendio» 3. Il censimento **segnala, non condanna**: nel Profilo alcuni «Gol» sono contesti diversi (stagione, carriera, una partita) e vanno guardati uno per uno.
- 21/09 — **G8.11 · il menù in basso è quello del provino: niente emoji** (rilievo PO «vedo ancora emoji e non la grafica proposta per i menù»). Il numero viene dal provino che il PO stesso ha approvato: **non ha una sola emoji** — i cappelli sono etichetta maiuscoletta + filo + azione, la barra in basso è **solo testo**. Il gioco ne rende **315** a 412 px. Prima la misura: la griglia conta ora i **caratteri emoji** per schermata e stampa i primi testi che li portano (peggiori: Calendario 64, Profilo 43, Dashboard 37, Club 37). Rifatta la barra sulla regola `.nav` del provino: Barlow Condensed, 11 px, maiuscolo, spaziato, voce attiva col filo di 3 px, **zero icone**; via anche la riga della scorciatoia da tastiera, che nel provino non c'è e su un telefono non serve (il tasto continua a funzionare, resta nel `title`). **Misurato: emoji 315 → 267** (−48 = le sei della barra su ognuna delle otto schermate di carriera), difetti invariati a zero. **Restano i cappelli di sezione**: 90 in `src/18` con l'emoji in testa, il passo successivo.
- 21/09 — **PRODUZIONE: la radice del sito rispondeva 404, e non era il codice.** Foto del PO. I sei deploy della giornata erano tutti **verdi**. La causa stava nella lista dei workflow: accanto a `Deploy to GitHub Pages` girava ancora **`pages-build-deployment`**, il deploy **legacy da branch**, che pubblica `main` **così com'è** — e in `main` un `index.html` non è mai esistito (lo **crea** il workflow dentro `_site`). Quando a pubblicare per ultimo era il legacy, la radice non aveva nulla da servire. Il commento in cima al workflow diceva «sostituisce il deploy legacy»: **non era mai stato spento**, e quella frase mi ha fatto cercare nel posto sbagliato. **Rete spedita subito** (nessun file di gioco toccato): `index.html` alla radice, `404.html` che porta al gioco, e un controllo che **ferma il deploy** se `_site` non contiene index, gioco, 404, manifest e service worker. **Correzione vera, fatta dal PO**: Settings → Pages → Source = «GitHub Actions» — che chiude anche il secondo problema, più grave del 404: il legacy pubblicava **tutto il branch**, compresi i documenti interni che il workflow tiene fuori dal sito dall'audit del 13/08.
- 21/09 — **7.956 · il freeze dei bottoni della scelta, i cappelli senza emoji, e tre ritocchi dal telefono.** **(1) Il freeze** (rilievo PO: «ridicolo, troppo lungo»): in `hl_choose` i bottoni nascevano `disabled` e si armavano dopo **1.200 ms**. La storia del numero conta, perché è la seconda volta che si muove: nasce a **450 ms** per un difetto vero (il dito che ha premuto «Scegli» o una freccia del D-pad **ricade** sul primo bottone e sceglie da solo), la **7.926** lo porta a 1.200 **su richiesta del PO** («un secondo o due, è il tempo di leggere»), oggi il PO lo boccia. **La nuova direttiva sostituisce la precedente**, e cade con essa la giustificazione del «tempo di leggere»: quello lo dà la scena, non un bottone spento. Resta solo la protezione dal rimbalzo del tocco, che è un fatto fisico: costante unica **`_ARMO956` = 250 ms**. **Misurato nel DOM** (non nel sorgente: la costante dice cosa ho scritto, il DOM dice cosa vede il dito) col guardiano nuovo `npm run armo-scelte`: **272-288 ms contro 1.200** del rosso `__CPM_NO956`, dentro la banda [120, 450]. **(2) I cappelli senza emoji**: il provino non ne ha nemmeno nei cappelli, e le sue fisarmoniche portano solo titolo, conteggio e freccia — tolte **86 emoji** in testa ai cappelli e le **12 icone** delle fisarmoniche. **Emoji rese 267 → 230** (erano **315** prima della 7.955). **(3) Tre ritocchi chiesti dal PO**: la mail delle idee va a **korward.devteam@gmail.com** e non più alla sua casella personale (l'offuscamento in codici di carattere resta: la casella nuova merita la stessa protezione dai raccoglitori di spam); **voce di menù più alta** (padding verticale 16 → 29, altezza misurata **49 px**, sopra i **44** consigliati per il tocco) e **striscia «Sostieni» più bassa** (padding 16 → 8, altezza **38 px**; la striscia «Idee» sta a 25). Metro nuovo: tabella **9-ter** della griglia. **Dichiarato**: sotto i 38 px del «Sostieni» c'è il marchio impilato su due righe, che non tocco senza il via libera del PO.
- 22/09 — **7.957 · «non è un sito ma un gestionale»: i box si stringono, e il 990 smette di essere una giornata.** **(1) Densità** (direttiva PO): imbottiture **−30 % in verticale** e −15 % in orizzontale (il respiro laterale serve alla riga, quello verticale era aria), più i ritmi fra i box e dentro i box. Due giri: 199 imbottiture, 235 ritmi, 83 distanze, 10 margini alti, e il valore di fabbrica della `Card` (16×18 → 11×15). **Misurato a 412 px**: Dashboard **2.700 → 2.542**, Profilo **2.444 → 2.198**, Club **1.527 → 1.416**, Agente 1.594 → 1.521, Calendario 1.651 → 1.602, Classifica 2.035 → 1.988, Nazionale 1.183 → 1.142. **E i difetti non si muovono** — 0 overflow, 0 fuori schermo, 0/2.444 sotto il pavimento, 0/1.980 sotto contrasto: comprimere fino a far toccare i bordi sarebbe barare sull'altezza pagando in leggibilità. **(2) Il 990 non è una giornata, è un codice** (rilievo PO «che significa 990ª giornata??»): le voci di coppa nascono con `matchday = 990 + turno` e la Nazionale con `1470 + settimana` — codifiche interne che la testata stampava come se fossero giornate. Ora la regola sta in un punto solo (`_etichettaTurno957`): in campionato la giornata, in coppa il **turno** (`cupRoundName`), in Nazionale niente. **(3) Il marchio su una riga** nella striscia «Sostieni», col via libera del PO: altezza **38 → 26 px**.
- 22/09 — **IL BANCO NON VEDE LE SCHERMATE CHE IL PO FOTOGRAFA, E VA DETTO.** In pochi minuti il PO ha segnalato **tredici** rilievi — formazioni «disomogenee», «il mondo fuori» fuori standard, l'intervista in 3D «terribile», il box del rivale «enorme», l'agente «particolarmente fuori standard». Diagnosi: non sono tredici difetti, sono **superfici rimaste scure** quando il tema scuro è stato ritirato (7.947), più componenti sovradimensionati. Aggiunto il **censimento delle superfici scure** alla griglia (riquadri larghi mezzo schermo, alti ≥ 40 px, luminanza < 0,25). **E il censimento le ha mancate**: trova **10 superfici**, ma sono la testata dell'eroe (`#6c1f2e`, voluta) e due bottoni di marca. «Il mondo fuori», «Il tuo rivale», «Club dei sogni», le formazioni e l'intervista **non compaiono**, perché vivono in stati che il salvataggio di prova non accende: è la **stessa cecità dei trofei** di G8.3, su un'altra famiglia. Il PO gioca una carriera alla **S.12** con rivale, club dei sogni e momenti accesi; il banco sta a una S.4 vuota. **Prima di correggere queste schermate va arricchito il salvataggio di prova**, altrimenti si corregge alla cieca.
...
- 22/09 — **7.960.0 · A14: la coppia che chiude il determinismo esiste, ed e' misurata — ma costa l'orologio** (§5.4). Tre pezzi provati uno alla volta. **Primo passo** (il ciclo dei sotto-tick si ferma con la fisica): motore 26' → **31'**, cronaca 27' → **34'**, passi di fisica identici per tutta la partita. **Terzo passo** (la richiesta di scena consumata sul battito del metronomo, non da un effetto di React): **da solo non vale** — 26/27/27 %, il rosso di partenza. **La coppia**: `prima-divergenza` **cronaca identica riga per riga**, nessun minuto con un sorteggio di differenza; `match-sequence` **2x↔2x 100 %**. **Revocata lo stesso**: fermando il contatore si ferma il battito del minuto, e il fischio finale scende da **4/4 a 1/4 giri**. Un metro spedito non si abbassa. Posizioni seedate rigiudicate sulla misura pulita e revocate una terza volta (27' con o senza). Nessun pixel toccato.
- 22/09 — **7.961.0 · la sala stampa e' disegnata** (collaudo PO «togli il 3d, fai un disegno carino 2D anche senza visi» + «togli i visi, lascia solo una scenografia 2D molto carina»). La 7.948 aveva tolto i **corpi** CH38 ma aveva lasciato in piedi proprio le due cose indicate: la scenografia era ancora la **mixed zone 3D** (un canvas WebGL) e sopra ci stavano **due visi** della libreria. Ora e' tutto disegnato in CSS: pannello step-and-repeat coi due colori del club e il marchio del gioco alla taglia giusta, riga d'appoggio, moquette, due microfoni in primo piano. **MISURATO** (`intervista-2d`, contratto rifatto): canvas WebGL **1 → 0**, visi nella scena **2 → 0**, corpi CH38 0 → 0, errori di pagina 0; rosso appaiato `__CPM_NO963` riporta la scenografia 3D in scena. Tre stesure, ognuna corretta su uno **scatto**: cinque piastrelle per riga si tagliavano sul bordo (→ tre, con le righe dispari a due), le aste dei microfoni finivano a mezz'aria (→ corrono sotto il modale), e un commento JSX scritto male **si leggeva sullo schermo** (trovato guardando la foto, non il codice).
- 22/09 — **7.962.0 · A14 chiusa a meta', e la meta' chiusa e' quella che conta** (§5.4). `match-sequence` era rosso in produzione da settanta versioni: ora **1x↔1x 100 %**, **2x↔2x 100 %**, 1x↔2x 87 %, col fischio finale ancora **4/4**. Tre tagli misurati (ingresso in scena sul battito · fisica indipendente dalla fase · debito di passi pagato al battito) e **due revoche** a verbale. Trovato per strada: **`ball-alive` e `trama` sono rossi e ciechi anche in produzione** e non sono nella catena `ci` — misurano il vecchio mover, non il motore: voce **A15**. Nessun pixel toccato.
- 22/09 — **7.963.0 · G9 · il censimento dei CARATTERI RESI, e mezzo gioco era fuori standard**. Rilievo PO «la schermata iniziale e' rimasta completamente fuori standard»: la griglia misurava contrasto, corpo, raggio e tinta, **mai la famiglia**. Tabella nuova **9-quinquies** (prima famiglia della `font-family` *calcolata*, per nodo di testo visibile). **MISURATO**: Creazione **544 nodi in Arial** contro 275 in Barlow, Classifica 5 in Arial, `Segoe UI` su otto schermate — **552 nodi fuori standard**. Due cause: i **controlli non ereditano** il carattere (contratto del browser: `input/select/textarea/button`) e il **marchio** era in `Segoe UI`, che non e' incorporato — sul telefono del PO cadeva sul ripiego di sistema. Due righe. **RI-MISURATO: 552 → 0**, tutte e tredici le schermate su **una sola famiglia, Barlow** (Creazione 819/819), senza regressioni (overflow 0 · fuori schermo 0 · pavimento 0/1978 · contrasto 0/2457 · completamento medio 89,7 % invariato).
- 22/09 — **7.964.0 · G10 · il momento e' una fisarmonica** (rilievo PO «ancora tanti box molto lunghi, farei accordion» + «la schermata e' ancora lunghissima»). `Card` ha un modo **fisarmonica** (`momento`): il riquadro si presenta chiuso — una riga col titolo e la freccia, `aria-expanded`, sopra il bersaglio tattile — e il contenuto arriva al tocco. Nove riquadri narrativi della Dashboard convertiti. **MISURATO: 2718 → 1890 px**, cioe' **2,97 → 2,07 schermate**; completamento della schermata **80,0 → 85,9 %**; raggi 7 → 5, corpi 10 → 9; **media delle tredici 89,7 → 90,2 %**. Zero difetti (overflow 0 · fuori schermo 0 · pavimento 0 · contrasto **0/115** · zero errori di pagina). Due difetti trovati **dalla misura** e chiusi prima di spedire: l'inchiostro bianco dichiarato anche sulla card a fondo chiaro (**1,00:1**, bianco su bianco) e il corpo della riga lasciato al ripiego del browser (un `button` non eredita la taglia del testo). La Dashboard non e' piu' la peggiore del metro: ora e' il **Profilo** (83,0 %, 2,58 schermate, 12 raggi).
- 22/09 — **7.965.0 · G11 · i raggi erano dodici, ora sono quattro**. (a) Il **Profilo** diventa una lista di sezioni: cinque riquadri a fisarmonica — fra cui **il box del rivale**, che il PO aveva chiamato «enorme» — **2361 → 2023 px** (2,58 → 2,21 schermate), **83,0 → 84,8 %**. (b) I **RAGGI**: il provino ne dichiara tre (3 · 6 · 50 %) piu' la scala dei token, il gioco ne rendeva fino a **dodici sulla stessa schermata**, perche' duecento erano scritti a mano accanto al token che gia' esisteva. **204 valori agganciati alla scala** (1,2→3 · 4,5,7→6 · 9,10,11→8 · 13,14→12 · 18,22→20). **RI-MISURATO**: Profilo 12→6 · Agente 7→4 · Dashboard 7→4 · Club 6→5 · Classifica 5→4 · Calendario 5→4 · Creazione 5→3 · Impostazioni 5→3 · Home 5→4 · Prepartita 5→3 · Offerte 3→2. **MEDIA DELLE TREDICI 89,7 → 92,3 %**, Impostazioni a **100**. Zero difetti nuovi e zero errori di pagina.
- 22/09 — **7.966.0 · G12 · lo spazio dei volti e' rettangolare e verticale** (direttiva PO: figurine stile Panini in arrivo dal team codex). Nessun disegno: lo **spazio** e il **contratto**. `FIG` (rapporto **5:7**, in un posto solo), `voltoUrl()` (manifesto `window.__CPM_VOLTI` → convenzione `assets/volti/<tipo>/<chiave>.webp`; senza manifesto **nessuna richiesta alla rete**), `Figurina` (rapporto tenuto a ogni larghezza, `object-fit:cover`, ripiego col volto di oggi). **Sei spazi in piedi**: il ritratto dell'eroe nella testata (larghezza **31**, perche' 31×7/5 = 44 = l'altezza esatta del tondo di prima — **la testata non cresce di un pixel**) e i cinque del procuratore. **MISURATO** (tabella nuova **9-sexies**): otto schermate con la figurina, **scarto dal 5:7 = 0,9 %** (l'arrotondamento ai pixel interi), **media delle tredici 92,3 % invariata**, zero errori di pagina. Contratto per chi disegna in **`docs/FIGURINE-VOLTI.md`** (consegne 2x/4x, occhi al 38 %, spalle tagliate, margine 6 %, fondo mai trasparente, nomi file, consegna **a lotti**). Non toccate: l'intervista (il PO ha chiesto **nessun volto**) e l'HUD di partita (vuole la catena completa).
- 22/09 — **7.967.0 · G13 · la scala dei corpi**. Stesso metodo dei raggi: il provino dichiara **sei** corpi, le schermate ne rendevano fino a **undici**, perche' **276 taglie** erano scritte a mano accanto al token (14 ×62 · 11,5 ×52 · 16 ×35 · 22 ×31 · 18 ×29 · 12,5 ×28 · …). Agganciate a `FS`, quasi tutte **verso il basso** — la direzione che il PO ha chiesto. **RI-MISURATO**: Agente 11→8 · Club 10→8 · Calendario 10→8 · Dashboard 9→8 · Classifica 9→7 · Nazionale 8→7 · Coppe 7→6 · **Profilo 7→6 (il bersaglio)** · Prepartita 4→3. **MEDIA DELLE TREDICI 92,3 → 93,2 %**. Zero difetti nuovi (overflow 0 · fuori schermo 0 · pavimento 0 · contrasto 0) — e lo dice la misura, non il ragionamento: agganciare 276 taglie poteva far sporgere una riga ovunque. **Restano le TINTE** (Dashboard 14 · Classifica 13 · Profilo 12 contro le 5 del provino): li' non si aggancia alla cieca, perche' fra quelle ci sono i colori **semantici** — prima si misura quali lo sono davvero.
- 22/09 — **7.968.0 · G14 · quattordici tinte non erano quattordici concetti**. Censimento nuovo (**9-septies**: le tinte del testo una per una, coi nodi e un esempio). **MISURATO**: erano **tre verdi**, **quattro ambre**, **tre blu** e **due rossi** — e quasi nessuno di quei valori sta nel sorgente: li fabbrica `semTesto945`, che scurisce una tinta semantica a passi del 16 % finche' il contrasto non arriva a 4,5:1, cosi' lo stesso verde su due fondi esce con due valori. Il contrasto era giusto, l'esito no. **Rimedio**: il risultato si **aggancia a un elenco di inchiostri approvati**, uno per famiglia, e solo se e' davvero vicino e passa lui stesso il 4,5:1 — altrimenti vince il valore calcolato (il contrasto non si baratta con l'ordine). Rosso `__CPM_NO968`. **RI-MISURATO**: Profilo **12 → 9** · Classifica **13 → 11** · Dashboard 14 → 13 · Club 11 → 10; **media 93,2 → 93,4 %**; contrasto sotto soglia **0** su tutte e tredici. Residuo dichiarato: i due grigi della gerarchia (muted e faint) sono una scelta del tema, e i colori dei **club** resi come testo sono dati.
- 22/09 — **rapporto delle figurine CONFERMATO dal PO: 5:7** («rapporto va bene adesso»). A verbale in `docs/FIGURINE-VOLTI.md`: il team codex puo' disegnare su quel formato.
- 22/09 — **7.969.0 · G13-bis · il corpo misterioso era uno sbaglio dello strumento**. Tabella nuova **9-octies** (i corpi uno per uno, **col nodo che li porta**) e il «27» che nessuno trovava nel sorgente si e' nominato da solo: la **sigla dello stemma**, `<text fontSize="27">` dentro un `<svg viewBox>`. Li' dentro il 27 e' un'**unita' di disegno**, non un pixel — lo stemma e' alto 18-40 px, quindi quella sigla rende a 6-11. Il testo dentro SVG esce dal conto dei **corpi** e dal **pavimento** (grandezze in pixel) e resta nel **contrasto** (un rapporto, che non dipende dalla scala). ⚠️ **Il salto della media 93,4 → 93,8 % e' in buona parte una misura piu' onesta, non un gioco piu' bello: si dichiara, non si incassa.** Spedito anche cio' che cambia i pixel davvero: le ultime tinte gemelle agganciate (7 `#b45309`→`#92400e`, 5 `#2563eb`→`#1e40af`), la tinta della **fase di carriera** fatta passare dal correttore, e i **corpi calcolati** (anello OVR) che ora atterrano sul gradino piu' vicino della scala. Corpi resi: Club 8→7 · Dashboard 8→7 · Classifica 7→6 · Calendario 8→7 · Creazione 4→3 · Offerte 4→3.
- 22/09 — **7.970/7.971 · l'HUD di partita esce dal rosso, e sette sezioni lunghe diventano fisarmoniche**. (a) Col metro riparato (A16) l'HUD era **l'unica schermata del gioco con nodi illeggibili**: **8/24** contro **0** sulle tredici di carriera. Il fioco del vetro faceva **2,65:1** su «possesso» e «Statistiche», e le **sigle dei club** sul vetro **1,52:1** col granata — il peggiore di tutto il gioco. Inchiostri alzati (`#cbd5e1`) e sigle passate da `semTesto945`, che conserva la tonalita'. ⚠️ **L'opacita' del vetro non si tocca**: e' una decisione del PO (7.922). (b) Sette fisarmoniche nuove su Home e Stagione (rilievi PO «la home e' ancora incasinata», «anche qui ci vorrebbe un po' di organizzazione, accordion»): **Classifica 2,17 → 1,91 schermate, 89,8 → 93,1 %**; media delle tredici **93,4 → 93,9 %**. ⚠️ **La Dashboard non si e' mossa e va detto perche'**: il salvataggio del banco e' una carriera corta, quindi notizie, voci di mercato e duello capocannoniere non compaiono — gli screenshot del PO sono una **stagione 12**. Il metro ha un buco di **rappresentativita'**, non di misura: il salvataggio di prova va portato a carriera avanzata. (c) Correzione del PO accettata: **il banner «Gioca vs» che si muove e' voluto**.
- 22/09 — **7.972/7.974 · via tutte le faccine, e il salvataggio del banco diventa quello del PO**. (a) La **carriera di prova** passa dalla stagione 4 alla **stagione 12** con registro e archivio: il PO vedeva notizie, voci di mercato e duello capocannoniere che sul banco **non venivano nemmeno resi**. ⚠️ Tutti i numeri di altezza citati prima del 22/09 sono su carriera corta e non si confrontano. (b) **«La tua settimana»** — 473 px, il **25,8 %** dell'intera Dashboard — diventa fisarmonica: **Dashboard 1837 → 1460 px, 2,01 → 1,60 schermate**. (c) Direttive PO: **via tutte le faccine della libreria SVG** (19 punti convertiti, **25 → 0** volti resi) e **lo spazio 5:7 bianco neutro**. (d) La **figurina d'esempio** ricevuta cambia due regole: `object-fit: contain` (l'arte porta la sua cornice e la fascia col nome a filo del bordo) e **nessuna sovrapposizione** del componente quando l'arte c'e'. **Media delle tredici 93,9 → 95,2 %**, zero difetti.
