...
- **16/09 13:20** — **7.920.0 sul ramo, rituali in corsa** — *la riga si chiude quando `main` è allineato*. **C4 seconda ondata: i raggi, e il guardiano che tiene il sistema.** La 7.919 ha portato a zero i corpi sotto il pavimento tipografico, ma senza un guardiano il primo `fontSize:10` scritto domani riapre il buco e nessuno se ne accorge finché il PO non fotografa: ora c'è **`npm run design-system`** (statico, un secondo, **dentro il `ci`**), che boccia i corpi sotto il pavimento dichiarato e i raggi scritti a mano che **hanno già un token**. E i raggi: **222 punti** scrivevano a mano un valore che il sistema già definisce (`borderRadius:8` dove esiste `RAD.sm`). **Misurato: raggi presi dal sistema 80 su 534 (15,0 %) → 302 su 534 (56,6 %)**; corpi dal sistema 46,4 %; colori dal tema 45,1 %. **Nessun cambiamento visivo è possibile, per costruzione**: ogni sostituzione mappa il letterale sul token con lo **stesso valore** (`RAD.xs` vale 6, `RAD.sm` 8, `RAD.md` 12, `RAD.lg` 16, `RAD.xl` 20, `RAD.pill` 999). I **232 raggi che restano** a mano sono quelli senza token corrispondente (1-5 micro, 22-40 pillole e schede): dichiarati, sono la terza ondata insieme ai **colori** (3.071 esadecimali contro 2.521 usi del tema).
