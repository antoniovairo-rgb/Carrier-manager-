...
| cosa | numero | fonte |
|---|---|---|
| produzione (`main`) | **a81a8b5 = 7.907.0** (7.889 → 7.907 in produzione: C8 panchina, C3 v4 HUD, D7 corpi leggeri; 7.896 revocata) | verbale 20:22 |
...
| C8 | **panchinari, mister e vice con il corpo CH38 alleggerito** (PO 15/09: «renderei GLB anche i panchinari ed il mister», «non devono volare») — v1 a corpo pieno misurata: 16/16 corpi ma triangoli 1,09 → 1,84 M (+68,6 %), figure a T sospese; persa nel riavvio; v2 in corso sul corpo a 14.622 triangoli (rosso `__CPM_NO904`) | corpi CH38 in panchina 0/16 → 16/16 · piedi entro 0,15u dalla seduta 16/16 · triangoli ≤ +25 % · fps banco rosso/verde · foto ravvicinate | **in produzione** (main e42d71a, 19:48): 16/16 corpi, piedi 0,77 → 0,12u dal pavimento, bacino 0,53 → 0,13u dalla seduta, triangoli +19,5 %, fps invariati; foto verificate |
| C3 v4 | dai collaudi del PO sul telefono: barra della pressione tornata visibile in movimento e nella scheda (il timer dell'esitazione 4/6/9 s era invisibile), cursore solo nel passo di movimento (44 px) con l'overlay vestito come la scheda, riga di aiuto, filo del possesso specchiato con gli stemmi dal 46', transizione 0,6 s ed etichetta «cambio campo» | 915 e 700: cursore nella scheda sì → no · barra della pressione no → sì · tasto del cursore 40 → 44 px · «Scegli» 36 → 52 px · stemmi e filo cambiano insieme · transizione 0 → 0,6 s · etichetta assente → presente | **in produzione** (main e42d71a, 19:48) |
...
| D7 | **librerie migliori, carta bianca (PO 15/09)** — 7.907: i 22 in campo, i portieri e l'arbitro usano il corpo CH38 alleggerito (14.622 triangoli contro 48.140, stesso file della panchina C8); contatore dei fotogrammi a schermo e interruttore TEMPORANEO «Corpi: leggeri / pieni» nel menu di pausa (rosso `__CPM_NO907`) | triangoli in scena 1.317.654 → **559.834** (−58 %) · scambio a caldo 2,2 s, 23 corpi invariati · 0 errori di pagina · **i fotogrammi veri li misura il PO sul telefono**: due numeri, stessa partita | **in produzione** (main a81a8b5, 20:22); interruttore da togliere a collaudo finito; primo piano del corpo pieno non riuscito (confronto a occhio col pieno non verificato) |
