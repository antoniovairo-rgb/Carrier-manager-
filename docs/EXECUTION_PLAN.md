...
|---|---|---|---|
| **C1 — Semantic/Motion** | **#16 Semantic backbone ✅** = node test `cine-backbone` su TUTTE le 179 situations × azioni (deriveHL→buildHLTimeline→validateHLTimeline: possesso continuo, no teleport, testa⇐cross, n.passaggi, chiusura conclusiva) + loader `situations.mjs` reso FEDELE (popola intent/ballState/cine). **#17 Motion ✅** (gate). | AC-031…040, 071…080 (logic ✅) | L · **#16/#17 fatti** |
...
| **C3 — FI/NPC** | **#24 reattività difensiva ✅** (in `motion`). **#19 FI plausibilità ✅ + #30 piede preferito ✅** = node test `test:logic` su `decideExecution` (puro/deterministico): determinismo, no esiti impossibili, forte+libero≫debole+pressato, piede debole penalizzato. #25 arbitro/panchine/GK → AI Vision/perceptual | AC-061…070 (logic ✅), **081…090 (motion ✅)** | L · **#19/#24/#30 fatti** |
| **C4 — AI Vision ponte** | **#35 Situation Recognition ✅** (run su 5.38.0: 4/4 `matchesExpected=true`, conf 70-75) + **#18 camera perceptual ✅** (scorata 50-65). Run reale: quality medio 55.6, 4×WARNING (second-level, non blocca CI). **#34 cross-validation gate↔AI ✅** (`cross-validate.mjs`: divergenze gate-PASS↔AI-WARNING = ciò che il geometrico non vede, cap. 16). **#37 build comparison ✅** (`vision-compare.mjs` + `vision-baseline.json`: Quality Delta con tolleranza varianza modello, cap. 17). #36 video continuo → deferito (multi-fase già approssima) | AC-161…170 (AI Vision ✅) | M · **#18/#34/#35/#37 fatti** |
