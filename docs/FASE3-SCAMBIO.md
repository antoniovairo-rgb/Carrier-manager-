...
- **Produzione allineata a `2b04fb5` (7.979.0) il 23/09/2026, rituale completo verde.** Allineamento notturno: il ramo e `main` erano **gia' sullo stesso commit**, quindi il push e' stato un `Everything up-to-date` — ma la verifica **non** e' stata saltata, ed era il punto: il build in produzione aveva superato la **catena grafica mirata**, non il **gate completo**. Eseguiti sull'HEAD esatto, senza ricostruire nulla: `career-critical` **PASS** (exit 0) e `npm run ci` **PASS** (exit 0, **10 rituali**) — sistema grafico, nessun corpo sotto il pavimento degli 11 px, nessun raggio a mano che duplichi un token, impulsi nel contesto giusto, typing-shortcuts, save-compat, replay, **partita-vera con le bande d'accettazione che tengono**, maxischermo ancorato in ogni impianto. Albero pulito, `check-src` IDENTICO. **NON VERIFICATO**: e' Chromium headless a 412x915, **non l'Android del PO**.
