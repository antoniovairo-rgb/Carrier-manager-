...
| 1.3 | ✅ **FATTO (scaffold)** — **Packaging Capacitor**: `package.json` + `capacitor.config.json` (appId `com.elevora.football`, `webDir=dist`, splash), script npm, bridge **storage nativo** in build, **icona/splash granata** (`resources/` + `tools/gen-assets.mjs`), **CI** `.github/workflows/android-build.yml`. Guida in `PACKAGING.md`. *Resta:* compilare l'AAB su PC/CI con Android SDK | M | Basso-medio | **P0** |
...
| 1.9 | ✅ **FATTO (grafica)** — **Asset listing**: testi (`STORE_LISTING.md`) + **icona 512** e **feature graphic 1024×500** granata (`store-assets/`, `tools/gen-store-graphics.mjs`) + **≥2 screenshot** grezzi dal gioco (`store-assets/screenshots/`, `tools/gen-screenshots.mjs`). *Resta (opz.):* rifinire/sostituire gli screenshot con catture su device | M | Basso | **P1** |
