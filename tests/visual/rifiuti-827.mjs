...
for(let g=0;g<NOMI.length;g++){
  ...
  console.log('  '+NOMI[g]+': righe nel diario '+d.cro.length+' · rifiutate da addCom (scheda aperta) '+d.ref+' · di cui di PIANO '+d.piano+' su '+d.beat.length+' battute · occasioni complete nel diario '+comp+'/'+occ.length+' · guardia 7.812 scattata '+d.salva+' volte · all\'emissione: battute '+(d.emit?d.emit.piano:'?')+', testo cambiato '+(d.emit?d.emit.cambiata:'?')+' · vive a p1/p2/p3: '+(d.emit?(d.emit.p1|0)+'/'+(d.emit.p2|0)+'/'+(d.emit.p3|0):'?'));if(d.j)console.log('      [J] palle morte armate dall\'occasione '+d.j.armati+' (corner '+d.j.corner+') · righe di corner uscite '+d.j.righe);if(d.nome)console.log('      [H] gol nostri al rigo '+d.nome.gol+' (loro '+(d.nome.loro|0)+') · con piano '+(d.nome.conPiano|0)+' · firmati dal protagonista dell\'ultima battuta '+d.nome.firmati+' · gol nel registro: '+(d.golEv||[]).join(' '));if(d.emit&&d.emit.es)d.emit.es.forEach(e=>console.log('      '+e.min+"' era «"+e.era+'» ora «'+e.ora+'»'));
}
