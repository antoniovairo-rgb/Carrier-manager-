...
for(let g=0;g<NOMI.length;g++){
  ...
  const d=await page.evaluate(()=>({ref:window.__CPM_SC681_REF|0,piano:window.__CPM_REF_PIANO|0,salva:window.__CPM_SALVA812|0,emit:window.__CPM_EMIT812||null,beat:(window.__CPM_BEAT792||[]).filter(x=>x&&x.occ),cro:(window.__CPM_CRO802||[]).map(x=>x.txt)}));
  await ctx.close();
