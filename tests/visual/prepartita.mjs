...
try { await page.getByRole('button', { name: /Formazioni/ }).first().click({ timeout: 4000 }); await sleep(1800); await page.screenshot({ path: path.join(out, `formazioni${TAG}.png`), fullPage: true });
  await page.getByText(/Ingresso in campo/).first().click({ timeout: 4000 }); for (let k = 0; k < 6; k++) { await sleep(1500); await page.screenshot({ path: path.join(out, `ingresso-${k}${TAG}.png`) }); } } catch (e) { R.formErr = String(e.message).slice(0, 80); }
