...
try { await page.getByRole('button', { name: /Formazioni/ }).first().click({ timeout: 4000 }); await sleep(1800); await page.screenshot({ path: path.join(out, `formazioni${TAG}.png`), fullPage: true });
  await page.getByText(/Ingresso in campo/).first().click({ timeout: 4000 }); R.ingresso = []; for (let k = 0; k < 10; k++) { await sleep(900); R.ingresso.push(await page.evaluate(() => { try { return { st: window.__CPM_HYPER_CASUAL_STATUS, audit: (window.__CPM_CGTRADER_SCALE_AUDIT || []).length, masc: window.__CPM_HYPER_MASCOTS || null, pol: window.__CPM_HYPER_INTRO_POLICY || null, fase: window.__CPM_PHASE && window.__CPM_PHASE() }; } catch (e) { return null; } })); await page.screenshot({ path: path.join(out, `ingresso-${k}${TAG}.png`) }); } } catch (e) { R.formErr = String(e.message).slice(0, 80); }
