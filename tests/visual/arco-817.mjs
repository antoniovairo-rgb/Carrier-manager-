...
for(let g=0;g<NOMI.length;g++){
  ...
  const fps=await page.evaluate(()=>Math.round(window.__CPM_FPS708||0));const sost=await page.evaluate(()=>window.__CPM_SOST806|0);
  await ctx.close();
  battute.forEach(a=>TUTTE.push(a));
  console.log('    fps del banco ~'+fps+'  · archi sostituiti in volo (7.806): '+sost);
