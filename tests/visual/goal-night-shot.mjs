...
await sleep(4500);
for (let i = 0; i < 4; i++) { await sleep(1200); const buf = await canvasShot(page); fs.writeFileSync(path.join(OUT, `goal-night-${i}.png`), buf); }
console.log('→ out/goal-night-*.png · pageerr', errs.length ? errs.slice(0, 3) : 0);
