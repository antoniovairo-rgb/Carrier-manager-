/* [7.8.26 collaudo tester «porta disegnata sul muro / cartelloni non visibili»] stadio PICCOLO (Neroverde 'sas',
   come lo screenshot) di GIORNO, fase PLAYING → cattura la porta LONTANA: col nuovo FONDALE scuro dietro la rete la
   porta deve leggersi come CAVITÀ 3D (non un contorno piatto sul muro) in ogni frame d'attacco. */
...
await page.addInitScript((wk) => {
  ...
  const save = { phase: 'career', player: { name: 'Diag', nation: 'Italia', avatarId: 0, proStatus: 'pro', season: 2, week: wk, age: 24, ovr: 78, tutorialDone: true,
    club: { id: 'sas', n: 'FC Neroverde', a: 'SAS', p: 68, c: '#15803d', c2: '#0f0f0f', nat: '🇮🇹', lg: 'Lega A' },
...
for (let i = 0; i < 16; i++) {
  ...
  fs.writeFileSync(path.join(OUT, `goal-backdrop-${i}.png`), best);
}
if (best) {
  const png = PNG.sync.read(best); const x0 = Math.floor(png.width*0.28), x1 = Math.floor(png.width*0.72), y0 = Math.floor(png.height*0.28), y1 = Math.floor(png.height*0.46), sc = 4;
  ...
  fs.writeFileSync(path.join(OUT, `goal-backdrop-zoom.png`), PNG.sync.write(out)); console.log('zoom salvato');
}
console.log('→ out/goal-backdrop-*.png · pageerr', errs.length ? errs.slice(0, 3) : 0);
