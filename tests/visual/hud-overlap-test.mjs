...
await page.addInitScript(r => { window.__CPM_GLB = false; if (r) window.__CPM_NO548 = 1; }, ROSSO ? 1 : null);
...
const m = await page.evaluate(() => {
  ...
  return { ban, pan, cor: corR, sovr: inter(ban, pan), gap: (ban && pan) ? +(ban.t - pan.b).toFixed(1) : null,
...
await page.screenshot({ path: '/tmp/claude-0/-home-user/6dd74479-f58d-5f3f-a846-19342f6001fd/scratchpad/hud-' + (ROSSO ? 'rosso' : 'verde') + '.png' });
...
if (ROSSO) {
  const bad = !(m.sovr > 0 || !bgPieno);
  console.log(bad ? '❌ prova del rosso fallita: senza rimedio le voci NON si accavallano (il guardiano non misura nulla)' : '✅ prova del rosso: col rimedio spento le voci si accavallano e il coro torna nudo');
  process.exit(bad ? 1 : 0);
}
const bad = !(m.sovr === 0 && m.gap >= 4 && m.corFs >= 14 && bgPieno && m.intrusi.length === 0);
console.log(bad ? '❌ le voci si accavallano, il coro non è leggibile o una voce vive fuori zona' : '✅ voci separate (0 px² di sovrapposizione), coro con fondo proprio, nessuna voce fuori zona');
process.exit(bad ? 1 : 0);
