...
const here = path.dirname(fileURLToPath(import.meta.url)); const out = path.join(here, '..', 'character-lab', 'telecronisti'); fs.mkdirSync(out, { recursive: true });
...
R.voce = null; const t0 = Date.now();
while (Date.now() - t0 < 240000 && !R.voce) { await page.evaluate(() => { const p = document.querySelector('[data-cpm=pop919]'); if (p) { const b = p.querySelectorAll('button')[0]; if (b) b.click(); } const ph = window.__CPM_PHASE && window.__CPM_PHASE(); if (ph === 'hl_choose' && window.__CPM_RESOLVE) window.__CPM_RESOLVE(0); if (ph === 'hl_result') { const x = [...document.querySelectorAll('button')].find(y => /Continua/i.test(y.textContent || '')); if (x) x.click(); } }).catch(() => {});
  R.voce = await page.evaluate(() => { const v = document.querySelector('[data-cpm=voce23]'); if (!v) return null; const f = v.querySelector('[data-cpm-figurina]'); const r = v.getBoundingClientRect(); let o = 1, e = v; while (e) { o *= +getComputedStyle(e).opacity; e = e.parentElement; } if (o < 0.6 || r.width < 10) return null; return { rect: [Math.round(r.top), Math.round(r.height)], opacita: +o.toFixed(2), testo: v.innerText.replace(/\n/g, ' | '), figurina: f ? f.getAttribute('data-cpm-figurina') : null, img: !!(f && f.querySelector('img')) }; });
  if (R.voce) await page.screenshot({ path: path.join(out, `voce${TAG}.png`) }); else await sleep(300); }
