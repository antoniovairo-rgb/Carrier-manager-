...
function partita(seed){
  ...
  const T={home:VUOTA(),away:VUOTA()};let vivo=0;
  for(let t=1;t<=MIN;t++){
    const evs=m.tick({min:t});const st=m.stato();
...
console.log(`\n=== TABELLINO per squadra, media su ${N} partite (${2*N} tabellini) — motore di oggi contro partita vera ===`);
