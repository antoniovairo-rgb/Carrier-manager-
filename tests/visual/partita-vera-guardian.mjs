...
const motoreOn = rows.some(r => r.rk === 'motore');
if (motoreOn) { try { const _h = {}; for (const r of rows) if (r.rk === 'motore') _h[r.mk || '-'] = (_h[r.mk || '-'] | 0) + 1; console.log('  · righe del motore per fatto: ' + JSON.stringify(_h)); } catch (_e) {} }/* [7.870] il censimento dei fatti raccontati: un rosso su una banda si legge da qui *//* [7.870] col motore del possesso acceso le quattro bande della Fase 3 (mente/riga-fatto/raccoglitore/interruzioni da riga) si giudicano sui FATTI EQUIVALENTI del motore: le vecchie macchine sono spente per costruzione, la soglia non cambia */
const fischi = motoreOn ? Math.max(rows.filter(r => r.rk === 'motore' && /^(fallo|rigore|rimessa|corner|rinvio|fuori)$/.test(String(r.mk || ''))).length, rows.filter(r => r.rk === 'motore' && r.mk === 'battuta').length) : turni.filter(t => /^interruzione-/.test(t.causa || '')).length;/* ogni battuta e' la ripresa dopo un'interruzione: si conta il massimo fra fischi raccontati e battute raccontate, mai la somma *//* [7.870] col motore le interruzioni sono fatti raccontati (fallo/rigore/rimessa/corner/rinvio), non cause di turno: una punizione non cambia il possesso */
