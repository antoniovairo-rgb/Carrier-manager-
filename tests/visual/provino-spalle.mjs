...
const out=[];for(const clipN of ['idle','jog'])for(const corr of [0,1])for(const vista of ['fronte','lato']){const av=THREE.SkeletonUtils.clone(corpo.scene);sc.add(av);av.traverse(o=>{if(o.isMesh)o.frustumCulled=false;});
  ...
  const lat=(Math.hypot(Lp.x-N.x,Lp.z-N.z)+Math.hypot(Rp.x-N.x,Rp.z-N.z))/2,pend=Math.atan2(N.y-(Lp.y+Rp.y)/2,lat)*180/Math.PI;
  ...
  const f=document.createElement('figure');f.style.margin='0';const im=new Image();im.src=ren.domElement.toDataURL();f.appendChild(im);const cap=document.createElement('figcaption');cap.style.cssText='color:#dde;font:12px sans-serif;text-align:center';cap.textContent=clipN+' · '+(corr?'CON':'SENZA')+' correzione · '+vista+' · pendenza '+pend.toFixed(1)+'°';f.appendChild(cap);document.getElementById('r').appendChild(f);
  out.push({clip:clipN,corr,vista,pend:+pend.toFixed(1)});sc.remove(av);}
