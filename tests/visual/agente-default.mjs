/* [23/09 POC] PROCURATORE DI DEFAULT: la carriera nuova ha gia' un procuratore, nessun «Fondi insufficienti». CPM_ROSSO=__CPM_NO_AGENTE23 */
const ROSSO = process.env.CPM_ROSSO || '', TAG = ROSSO ? '-rosso' : '';
import fs from 'node:fs'; import path from 'node:path'; import { fileURLToPath } from 'node:url';
import { startServer, launchBrowser, installCdnRoutes, sleep } from './lib/harness.mjs';
const here = path.dirname(fileURLToPath(import.meta.url)); const out = path.join(here, '..', 'character-lab', 'agente'); fs.mkdirSync(out, { recursive: true });
...
await page.evaluate(() => window.__CPM_CAREER.goTab('agente')); await sleep(1200);
const R = { rosso: ROSSO || null }; R.agente = await page.evaluate(() => ({ nessuno: /Nessun agente/.test(document.body.innerText), ingaggia: /Ingaggia un procuratore/.test(document.body.innerText) }));
await page.screenshot({ path: path.join(out, `agente${TAG}.png`) });
