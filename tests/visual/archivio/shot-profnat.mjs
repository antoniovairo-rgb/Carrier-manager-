...
await page.addInitScript(() => {
  ...
  const save = { phase:'career', player:{ name:'Francesco Vairo', nation:'Italia', avatarId:0, proStatus:'pro', season:12, week:14, age:26, ovr:85, tutorialDone:true,
    ...
    nationalCaps:24, nationalGoals:11, lastNationalSeason:12, seasonCards:{y:2,r:0}, yellAccum:1,
    euroMondiale:{active:true,type:'Europeo',phase:'group',groupPts:6},
    diary:[{type:'first_national',season:8,week:20,headline:'Esordio in Nazionale',color:'#003399'}],
...
await sleep(1200);
// go to Carriera nav → Profilo sub-tab
try{ await page.getByText('Carriera',{exact:false}).first().click({timeout:5000}); await sleep(900);}catch(e){console.log('carriera nav fail',e.message.slice(0,60));}
try{ await page.getByText('Profilo',{exact:false}).first().click({timeout:4000}); await sleep(1000);}catch(e){console.log('profilo sub fail',e.message.slice(0,60));}
// count national boxes
const cnt = await page.evaluate(()=>{
  const els=[...document.querySelectorAll('*')].filter(n=>n.children.length===0&&/Nazionale Italia/i.test(n.textContent||''));
  const rank=[...document.querySelectorAll('*')].filter(n=>n.children.length===0&&/^Ranking Mondiale$/i.test((n.textContent||'').trim()));
  return {naz:els.length, rank:rank.length};
});
console.log('COUNT', JSON.stringify(cnt));
await page.screenshot({ path:`${OUT}/profnat.png`, fullPage:true });
