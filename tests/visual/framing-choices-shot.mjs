...
/* PROVINI DELL'INQUADRATURA — la stessa conclusione a quattro distanze di camera, col numero accanto.
   ...
   COSA PRODUCE. Per ogni scena scelta, il fotogramma della CONCLUSIONE a quattro TAGLIE (manopola
   test-only `__CPM_FRAMET480`: si chiede l'altezza apparente voluta e la distanza si ricava invertendo
   la proiezione), con stampata accanto l'altezza MISURATA. Cosi' la scelta e' «voglio la riga da 0,30»
   e vale identica su tutte e 191 le scene, non «piu' stretto» — che scena per scena vuol dire cose
   diverse (misurato: lo stesso moltiplicatore 0,55 dava 0,115 su gi0 e 0,455 su gi7).
...
/* Si chiedono TAGLIE, non moltiplicatori: `0` = la regia di oggi, gli altri sono l'altezza apparente
   VOLUTA. Un moltiplicatore dava scene diverse fra loro (misurato: k=0,55 → 0,115 su gi0 e 0,455 su gi7),
   e su una decisione che vale 191 scene la riga scelta dev'essere la stessa ovunque. */
const ZOOM = (process.env.CPM_ZOOMS || '0,0.18,0.30,0.45').split(',').map(Number);
...
for (const gi of SCENE) {
  for (const k of ZOOM) {
    ...
    await page.evaluate(z => { window.__CPM_FRAMET480 = z || null; window.__CPM_FRAME480 = null; window.__CPM_FORCE_OUTCOME = 'success'; }, k);
    await page.evaluate(() => { try { window.__CPM_RESOLVE(0); } catch (e) {} });
    await sleep(2400);
    const f = await page.evaluate(() => window.__CPM_FRAME480 || null);
    const file = `${OUT}/gi${String(gi).padStart(3, '0')}_h${String(k || 'oggi').replace('.', 'p')}.png`;
    await page.screenshot({ path: file });
    righe.push({ gi, k, azione, alt: f ? f.max : null, fuori: f ? f.fuori : null, file });
    console.log(`  gi${String(gi).padStart(3)} · taglia chiesta ${String(k || 'oggi').padEnd(5)} → altezza MISURATA ${f && f.max != null ? f.max.toFixed(3) : ' — '} ${f && f.fuori ? `(fuori quadro ${f.fuori} fr)` : ''}`);
  }
}
await page.evaluate(() => { window.__CPM_FRAMET480 = null; window.__CPM_ZOOM480 = null; }).catch(() => {});
...
for (const gi of SCENE) {
  ...
  console.log(`\ngi${gi} «${r[0].azione || '—'}»`);
  for (const x of r) console.log(`   taglia ${String(x.k || 'oggi').padEnd(5)} · misurata ${x.alt != null ? x.alt.toFixed(3) : '—'}${x.fuori ? ' · esce dal quadro in ' + x.fuori + ' fr' : ''} → ${x.file.split('/').pop()}`);
}
