...
const N=+(process.env.CPM_PARTITE_G||4);
const OUT=[];
for(let g=0;g<N;g++){
  ...
  await openMatch(page,port,{skipLoadAll:true,name:'Gl'});
  ...
  OUT.push({g,score:sc,gol:gl.length,src:by,scene:es.length,min:(sc?null:'?')});
  console.log('partita '+g+': tabellone '+(sc?sc.h+'-'+sc.a:'?')+' · eventi gol '+gl.length+' '+JSON.stringify(by)+' · scene risolte '+es.length);
  await ctx.close();
}
