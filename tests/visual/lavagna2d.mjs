...
const here = path.dirname(fileURLToPath(import.meta.url)); const out = path.join(here, '..', 'character-lab', 'lavagna2d'); fs.mkdirSync(out, { recursive: true });
...
await sleep(1500);
R.lavagna = await page.evaluate(() => ({ heat: window.__CPM_HEAT23 || null, meteo: window.__CPM_METEO2D23 === undefined ? 'n/d' : window.__CPM_METEO2D23, campo: !!document.querySelector('[data-cpm=campo2d]') }));
await page.screenshot({ path: path.join(out, `lavagna${TAG}.png`) });
