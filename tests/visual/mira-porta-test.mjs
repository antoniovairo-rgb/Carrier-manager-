...
let righe = [];
for (let giro = 0; giro < 6 && righe.length < 8; giro++) {
  ...
  righe.push(...m.filter(x => Math.abs(x.tz) > 3.66));
}
await b.close(); srv.close();
const v = righe.map(x => x.porta).sort((a, c) => a - c), med = v.length ? v[v.length >> 1] : null;
console.log(`fotogrammi con la palla diretta fuori dai pali ${v.length} · angolo corpo-porta mediano ${med} gradi · gesti ${[...new Set(righe.map(x => x.g))].join(',')}`);
