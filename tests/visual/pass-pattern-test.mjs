/* [7.319.0] GUARDIANO PERMANENTE — DUE PROPRIETÀ DEL PIAZZATO E DEL PASSAGGIO
   ...
   (B) collaudo PO «sulle punizioni e rigori ci deve essere la rincorsa, non possono tirare da fermo!».
       Sul rigore il battitore ora arretra ~3.2 unità e corre sul pallone, arrivandoci quando la gamba
       colpisce. ⚠️ La PUNIZIONE è esclusa di proposito e il test lo asserisce: lì il timer del gesto
       risulta già avviato durante l'attesa, la rincorsa si consumerebbe prima del tiro e il battitore
       resterebbe congelato dietro il pallone. Resta da risolvere alla radice.
...
/* B — la rincorsa del rigore: arretra prima, arriva sul pallone al colpo */
const GP = await page.evaluate(() => { const S = window.__CPM_SITS || []; for (let g = 0; g < S.length; g++) { const s = S[g]; if (!s) continue; let hl = null; try { hl = window.deriveHL(s, (s.actions || [])[0]); } catch (e) { } if (hl && hl.type === 'penalty') return g; } return -1; });
if (GP < 0) { console.log('❌ nessun rigore nel pool'); fails++; }
else {
  ...
  say(attesa >= 2.6, `rigore · in attesa il battitore è arretrato di ${attesa}u dal pallone (atteso ~3.2)`);
  await page.evaluate(() => { window.__CPM_FORCE_OUTCOME = 'success'; window.__CPM_RESOLVE(0); });
  ...
  say(minimo <= 0.3, `rigore · durante la battuta la rincorsa si consuma fino a ${minimo}u (arriva sul pallone)`);
}

if (errs.length) { console.log('❌ pageerror:', errs.slice(0, 3).join(' | ')); fails++; }
console.log(fails ? `\n❌ FAIL — ${fails}` : `\n✅ PASS — il dato comanda sul testo, la palla non torna indietro, il rigore ha la rincorsa`);
