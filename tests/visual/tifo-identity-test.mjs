...
const out = await page.evaluate(() => {
  ...
  for (const c of sample) {
    ...
    rows.push({ id: c.id, n: c.n, nat: c.nat, main1, main2, banners });
  }
  return rows;
});
...
const trOk = tr.length ? tr.filter(r => /TRİBÜN|TARAFTARI|ÇOCUKLARI|KUŞAĞI|YÜREĞİ|SEVDASI|AŞK|YÜREK|GURURU|BERİ|RENKLER|SES|AŞKIMIZ|BİRLİKTE|KADAR/.test(r.main1)).length : 0;
...
console.log(issues.length ? '❌ FAIL\n' + issues.map(i => '  ✗ ' + i).join('\n') : '✅ IDENTITÀ ULTRAS OK (stabile per club · nessuna collisione · lingua coerente · zero nomi reali · leggibile)');
