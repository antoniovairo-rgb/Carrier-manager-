...
| BL-06 | Animation quality: blending/transizioni/fluidità. FATTO 7.49.0 (blending GLB): gesto precedente in fade (via il pop), attack/release asimmetrici, smoothstep idle↔corsa — collaudo dal vivo pendente; ulteriori rifiniture (transizioni di fase, clip aggiuntive) restano possibili | 7 | L | — | AIV + Live | ◑ (blending ✅) |
...
| ID | Titolo | Fase | Eff | Dipendenze | Verifica | Stato |
|---|---|---|---|---|---|---|
| BL-10 | Camera broadcast: tracking/framing/zoom/cinematic (cameraQuality 50-65→↑). 1° slice 7.54.1 (CAM-LEAD): in `playing` la WIDE anticipa la palla nella direzione del moto (velocità EMA → lead lookAt bounded, azzerato sui teleport di setup) — golden-safe per costruzione (fasi HL invariate). Prossime slice (zoom dinamico, cut d'esito) dopo collaudo PO | 9 | L | — | AIV + Live | ◑ (lead ✅) |
...
| BL-13 | `SAVE_VERSION` come gate di migrazione reale (+stamp). FATTO 7.53.1: migratePlayer timbra saveVersion a valle delle riparazioni + `migratedFrom` (origine, una sola volta) → migrazioni versioned possibili; probe save-stamp-test | 4 | M | — | save-stamp-test | ✅ |
...
| BL-15 | Sync cronaca↔esito↔animazione fine (AC-049/050). FATTO 7.54.0: il renderer segnala la fine dell'arco d'esito (`onOutcomeShown`) → reveal/cronaca dei non-gol all'IMPATTO reale (floor suspense `_rd92` + fallback 2600ms); cronaca gol/assist all'ingresso in rete. La parte «replay» di AC-050 dipende dal Replay Engine (BL-21) | 5 | M | timeline | AIV + timeline | ✅ |
...
| BL-19 | AI Vision: analyze() OpenAI/Gemini ✅ (4 provider) + SCORING STORICO ✅ (`lib/vision/history.mjs`, trend+sparkline) + **VIDEO CONTINUO ✅** (`VISION_MODE=video`: `captureHighlightSequence` cattura un FILMSTRIP ordinato di N frame — clamp 3..16 — della conclusione; prompt MOVIMENTO dedicato che giudica fluidità/continuità-traiettoria/tempi/scatti-teleport, schema JSON invariato → scoring uguale; cache separata frames/video; `test/vision/video-mode.test.mjs` 7, test:vision 43/43; probe browser: 8 frame ordinati 0 artefatti). Feature completa | 11 | M | — | test:vision | ✅ |
| BL-20 | Acceptance + Regression come step formali della CI + Failure Package ricco. 7.58.x: ogni failure del run-summary porta una **REPRO RECIPE** azionabile (`__CPM_RESEED(seed)` + `__CPM_FORCE_SIT(gi)` o run-command) · test `failure-package.test.mjs` (4). **7.60.x: REGRESSION HISTORY** (`lib/gate-history.mjs`) — accumulo build-to-build del fingerprint del gate in `gate-history.json` (rolling cap 200) + trend (regressione pulita→rotta · riparazione · fpChanged · clean-streak · sparkline) stampato dal runner; test `gate-history.test.mjs` (8). Restano: Acceptance/Regression come step CI **distinti** nel workflow | 12 | M | — | CI | ◑ (repro + history ✅) |
