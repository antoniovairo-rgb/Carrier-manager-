...
- **STADI DINAMICI — S1 fondamenta (5.87.0):** design in **`docs/STADIUM_SYSTEM_DESIGN.md`** (approvato dal PO: 3 slice · MAI pista d'atletica · trasferta invertita in S3 · evoluzione automatica). S1 = strato dati: `STADIUM_TEMPLATES` (10 preset) + `STADIUM_LG_TPL` (14 leghe→template) + **`stadiumConfigFor(club,evo)`** puro/seedato (capienza stabile per club dalle bande PO, nome stabile, evoluzione derivata, override data-driven `club.stadium{...}`) → alimenta `computeCrowdContext` (prima la capienza cambiava a ogni partita). Zero cambi visivi (assemblatore in S2). Test: `tests/visual/stadium-config-test.mjs`. ⚠️ In S2/S3: NON spostare il tabellone dal lato Sud (definisce la Curva Sud del charter pubblico).
