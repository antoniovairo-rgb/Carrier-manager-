...
- **FASE 3.3 — Agency e conseguenze (5.82.0, BIL-4/8 — chiude la Fase 3):** il **training si sceglie** (TrainPanel: piano del mister consigliato / offensivo / atletico / scarico; override medico automatico a fatica alta; auto-training all'avanzamento come fallback — prima si auto-eseguiva al mount) e il **rinnovo si può perdere** (`openNegoModal` → step `refused` con trust<40 o riserva+trust<52: svincolo reale, GIOCA bloccato finché non firmi altrove). ⚠️ Flussi carriera gate-ciechi → collaudo dal vivo.
