...
| 2639–2681 | `ARCHETYPES`, `GAME_VERSION` (**5.57.0**), `SAVE_VERSION` (**7**) |
...
- **cpmadapt — IA off-ball adattiva (5.58.0):** la difesa «si aspetta» lo schema ripetuto e **ombreggia** verso la fascia più attaccata dall'Eroe (Cap. 4.12). `matchMem.byFlank` traccia la fascia (da `pPosRef.current.y`); il render passa `adaptShift` (deterministico, **bounded ±4u**, dopo ≥3 azioni) e l'off-ball sposta la y dei difensori della squadra che difende (mai GK, sospeso sui set-piece). Default ON, **`?cpmadapt=0`** lo spegne. Gate-cieco → feel da collaudare dal vivo; test direzionale in `tests/visual/adapt-test.mjs`.
