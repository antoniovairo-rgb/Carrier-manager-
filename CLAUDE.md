...
**Invariante «una misura sulla luce si fa a ORA CONGELATA» (7.539.0):** il mondo ha `timeOfDay` (day/dusk/night) e le luci cambiano con lui — una sonda fotografica che non forza `__CPM_TOD` misura l'ora che le capita, e due bracci finiscono su ore diverse. E' costata una release: la nota «colori slavati» del 7.537 e' stata tarata su scene qualunque, quindi la NOTTE — l'ora in cui il PO gioca — non era mai stata misurata. Corollario di merito: «slavato» va misurato con **tre** numeri, non uno — saturazione, **luminosita'** e **contrasto**; nel caso vero il colpevole era la luce (+25/+40%) e non il colore (-3/-8%). Corollario di metodo: il braccio di RIFERIMENTO va ripetuto e mediato — fra due giri si muove del 3-4%, cioe' quanto l'effetto cercato. `npm run colori`.

**Invariante «un tick e' un MINUTO, non un secondo» (7.538.0):** il metronomo della partita (`MATCH_TICK_MS`, unico `setInterval`) avanza il cronometro di **un minuto per tick**. Ogni costante che vive dentro quel giro — passo del pallone, distanza dei waypoint, durata di uno spell di possesso, cooldown — si tara in **minuti di gioco**, mai in secondi reali: leggerla come un secondo ha prodotto un pallone che avanzava 0,8u al minuto (68u in 90') e una squadra che teneva palla per mezzo tempo. Corollario: **un cancello «e' arrivata?» va scalato col passo** (soglia proporzionale, non fissa) — a soglia fissa il motore del possesso girava una volta ogni sette minuti di gioco (40 tick di piano in tre partite) e la palla la muovevano tutti tranne lui. Rosso `__CPM_NO553`, misura con `npm run possesso`.

**Invariante «una misura su due sorgenti non e' una misura» (7.322 → 7.370 → 7.538.0):** la palla e i ventidue vanno letti dalla **stessa** sorgente. `__CPM_BALL`/`ball.position` sono MESH, `__CPM_MP` e' l'array **logico** `matchPlayers`: divergono di 15-20u, e confrontarli produce numeri che sembrano diagnosi. E' costato un «66% di palla senza padrone» dichiarato al PO e poi ritirato (il valore vero era 22%). Per il possesso ambientale esiste `__CPM_OWN` (palla, ventidue ed eroe tutti dalle mesh, senza proiezione ndc). Corollario di campionamento: su una partita **accelerata** non si campiona a orologio — un campione per **minuto di gioco**, altrimenti giri diversi coprono partite diverse e la varianza sembra del mondo mentre e' dello strumento.
