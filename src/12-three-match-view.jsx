...
const _mx47=clamp(Math.max(Math.min(_rm.position.x+_lead54,AWAY_GOAL_X-13),ball.position.x-2,(sr.current._arLandX!=null?sr.current._arLandX-1.5:-99)),-48,GOAL_LINE_X-4);/* [7.310.0] pavimento al punto d'atterraggio: la palla non arretra a cercare il ricevente */
      ...
      if(isWalk){
        ...
        for(let i=0;i<_players.length;i++){
          ...
          {const _ob=_tgPool[_tgN]||(_tgPool[_tgN]={});_tgN++;_ob.x=tx;_ob.y=ty;_ob.gk=false;_ob.hm=_hm;_ob.li=_li;_ob.att=_li>=8;_ob.def=_li>=1&&_li<=4;_tg.push(_ob);}
        }
        ...
        for(let i=0;i<_players.length&&!P.ceremony&&!P.shootout;i++){const t=_tg[i],pp=_players[i];if(!t||!pp)continue;/* [7.2.0] durante la premiazione il movimento off-ball è guidato dal blocco ceremony (giro/curva/podio) */
          ...
          if(pp.mesh===passTargetMesh||(pp.mesh._rcvT!=null&&pp.mesh._rcvT>=0)||(pp.mesh._failRunT!=null&&pp.mesh._failRunT>=0)||pp.mesh._celT371===1||pp.mesh._carry526!=null)continue;/* [7.523.0 PORTATORE] chi conduce/riceve e' fuori dal driver di formazione *//* [7.371.0] chi sta correndo a festeggiare non viene riportato in posizione dall'AI off-ball: due driver sullo stesso mesh = il difetto del 5.43.13 */
        ...
        if(!_readFreeze&&!_doSnap&&!P.ceremony&&!P.shootout){const _MINSEP=1.7;
          const _skipSep=(m)=>!m||m.gk||m.mesh===passTargetMesh||(m.mesh._rcvT!=null&&m.mesh._rcvT>=0)||(m.mesh._failRunT!=null&&m.mesh._failRunT>=0)||m.mesh._carry526!=null;/* [7.523.0] il portatore non si sballotta *//* [6.45.0 RC] non spingere chi riceve (_rcvT) o corre a vuoto (_failRunT): animOne li congela di proposito → venivano sballottati a metà ricezione */
        ...
        if(actType){
          ...
          if(actType==="shot"){
            if(P.hlVariant==="shot_chip"){// pallonetto: corpo indietro, tocco leggero di punta, salto all'indietro
              ...
              if(hero._torso){hero._torso.rotation.y=sw*0.5;}if(_rv575){hero.rotation.x=1.5*sw;if(hero._ring553)hero._ring553.rotation.x=Math.PI/2-1.5*sw;}/* [7.553.0] l'aureola resta in piano mentre il corpo gira: e' un segno di regia, non una parte del corpo */
