    ...
    const loop=()=>{
      ...
      if(ballArcActive){const _preT74=ballArcT;
        ...
        if(u>=1||_early74){
          ...
          if(typeof window!=='undefined'&&(_CPM_TEST||_SIT_TEST)){try{const _ae=(window.__CPM_ARCEND457=window.__CPM_ARCEND457||[]);if(_ae.length<40)_ae.push({u:+(+u).toFixed(2),early:!!_early74,bg:!!ballArcIsBG,ht:P.hlType||null,hs:P.hlSuccess,def:!!P.hlDef,rew:P.hlReward||null,kind:P.hlOutcomeKind||null,ph:P.matchPhase||null,bx:+ball.position.x.toFixed(1),sk:(P.hlSitKey!=null?P.hlSitKey:null),own:!!(P.hlDefTraj&&P.hlDefTraj.toOwnGoal),res:!!isResult,cc:!!(sr.current._ccx460)});}catch(_e){}}/* [7.459.0] IL BLOCCO DI COMPLETAMENTO HA UN TESTIMONE: e' qui che nasce il post-arco, e su 4 gol su 6 il post-arco non nasce. Se questo marcatore non compare, l'arco e' stato ucciso prima; se compare e il post-arco resta nullo, e' la catena di rami a non prenderlo (i campi dicono quale). */ballArcActive=false;_arcSrcX=null;_arcSrcZ=null;// 3DV-9: trigger animazione fine azione in base all'esito
