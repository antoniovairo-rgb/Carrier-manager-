...
const _mx47=clamp(Math.max(Math.min(_rm.position.x+_lead54,AWAY_GOAL_X-13),ball.position.x-2,(sr.current._arLandX!=null?sr.current._arLandX-1.5:-99)),-48,GOAL_LINE_X-4);/* [7.310.0] pavimento al punto d'atterraggio: la palla non arretra a cercare il ricevente */
      ...
      if(typeof window!=='undefined'&&window.__CPM_CAMT767ON){try{window.__CPM_CAMT767={tPx:+tPx.toFixed(1),tPy:+tPy.toFixed(1),tPz:+tPz.toFixed(1),tLx:+tLx.toFixed(1),tLy:+tLy.toFixed(1),tLz:+tLz.toFixed(1),kp:+kp.toFixed(4),kl:+kl.toFixed(4),sal:!!P.salienteOn,hl:+(camHL||0).toFixed(2),isHL:!!isHL,ph:P.matchPhase||null,tocc:(sr.current._tocc503||[]).slice(-6)};}catch(_e767){}}/* [7.767 strumentazione test-only] i bersagli della regia PRIMA del liscio: senza, «lo sguardo e' dieci unita' lontano dal pallone» non distingue un lerp lento da un bersaglio sbagliato */
