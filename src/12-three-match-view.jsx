...
function ThreeMatchView(props){
  ...
  useEffect(()=>{
    ...
    if(typeof window!=='undefined')window.__CPM_DECOR=()=>{try{
      ...
      if(typeof window!=='undefined'&&window.__CPM_FONDALE768){out.fondale=[];scene.traverse(o=>{if(!o.isMesh||!o.geometry||!/Plane/.test(o.geometry.type||''))return;const pr=o.geometry.parameters||{};if(!(pr.width>7&&pr.width<13&&pr.height>2.5&&pr.height<3.5))return;const wp=new THREE.Vector3();o.getWorldPosition(wp);const nd=wp.clone().project(camera);out.fondale.push({w:pr.width,h:pr.height,pos:[+wp.x.toFixed(2),+wp.y.toFixed(2),+wp.z.toFixed(2)],vis:o.visible,culled:o.frustumCulled,ro:o.renderOrder,col:o.material&&o.material.color?o.material.color.getHexString():null,tr:!!(o.material&&o.material.transparent),op:o.material?o.material.opacity:null,dw:o.material?o.material.depthWrite:null,dt:o.material?o.material.depthTest:null,ndc:[+nd.x.toFixed(2),+nd.y.toFixed(2),+nd.z.toFixed(3)],ndc2:(window.__CPM_PROJ767?window.__CPM_PROJ767(wp.x,wp.y,wp.z):null),parent:o.parent&&o.parent.type,layers:o.layers.mask,camLayers:camera.layers.mask,matVis:o.material&&o.material.visible,chainVis:(function(){let q=o,v=true;while(q){if(q.visible===false)v=false;q=q.parent;}return v;})(),inScene:(function(){let q=o;while(q.parent)q=q.parent;return q===scene;})()});if(window.__CPM_FONDALE768==='paint'&&o.material){o.material.color.setHex(0xff0000);o.material.opacity=1;o.material.transparent=false;o.material.depthTest=false;o.material.side=THREE.DoubleSide;o.material.needsUpdate=true;o.renderOrder=999;o.frustumCulled=false;}});}/* [7.768 strumentazione test-only] il fondale della porta: esiste, dove sta, come e' fatto */
    ...
    try{if(renderer.debug&&!(typeof window!=='undefined'&&window.__CPM_NO769))renderer.debug.checkShaderErrors=false;}catch(_e769b){}/* [7.769.0] vedi la nota all'aggancio dei modelli: la lettura sincrona dei log di compilazione e' il blocco di 2,6 s all'apertura della prima scena */
    ...
    if(typeof window!=='undefined')window.__CPM_RINFO769=()=>{try{let lights=0,sh=0;scene.traverse(o=>{if(o.isLight){lights++;if(o.castShadow)sh++;}});return {programs:renderer.info.programs?renderer.info.programs.length:null,calls:renderer.info.render.calls,tri:renderer.info.render.triangles,tex:renderer.info.memory.textures,geo:renderer.info.memory.geometries,shadow:!!renderer.shadowMap.enabled,lights,sh,chk:renderer.debug?renderer.debug.checkShaderErrors:null};}catch(e){return {err:String(e)}}};/* [7.769 strumentazione test-only] quanti programmi shader esistono e quando nascono: il profilo CPU dice che l'apertura di scena passa 2,6 s in getShaderInfoLog/getProgramInfoLog */
  ...
  const _grdTone99=(wfx==="snow")?0xdde3ea:(tod==="day"?_gA.clone().multiplyScalar(0.86).getHex():0x252831);/* [7.186.0 collaudo PO «c'è una striscia verde più chiara»] il piazzale diurno era VERDE-grigio (0x55604f) → accanto al prato leggeva come una seconda erba di tonalità diversa; ora è cemento scuro, coerente col bordo campo (e con la notte) */
    ...
    if(_useGLB&&typeof loadGLB==='function'){const _optGLB=u=>loadGLB(u).catch(()=>null);
      ...
      const _mkA=(proc,kit,appr)=>{
        ...
        const _on769=!(typeof window!=='undefined'&&window.__CPM_NO769);if(!_on769){try{window.__CPM_GLB_READY=true;}catch(_e){}}// [7.9.3] CH38 pronto → LiveMatch può fischiare l'inizio · [7.769.0] con la precompilazione il «pronto» si dichiara DOPO aver compilato (vedi sotto): il fischio e' gia' trattenuto dall'overlay pre-partita, e' il posto giusto per pagare i 3-4 s di compilazione invece che alla prima scena
        /* [7.769.0 — LA SCENA NASCEVA CONGELATA: 2,6 SECONDI DI COMPILAZIONE SHADER ALLA PRIMA INQUADRATURA. Rosso __CPM_NO769]
           Note KE del PO sulla 7.767 (63' e 41', SIT #156/#107): «palla congelata» (011), «reparto fermo» (006), «apertura:
           il pallone non e' ai piedi di nessuno, eroe a >=32u per 74 campioni» (001) e «SALTO di 37,6 unita' in 21 ms a 5,9 s».
           Tracciato in headless con il contatore dei fotogrammi e il profilo CPU: all'apertura della scena i programmi shader
           passano da 13 a 20 (i modelli CH38 entrano tutti in quadro: 12k → 1,09 milioni di triangoli) e il thread principale
           spende il 66% dei primi 4 s in getShaderInfoLog/getProgramInfoLog — cioe' compila sincrono. Per 1,5-2,5 s non si
           disegna NULLA (0-1 fotogrammi), l'eroe e la palla restano dov'erano nel gioco, e quando il disegno riprende il tempo
           di scena e' gia' avanzato: il pallone «salta» dove il racconto lo ha portato nel frattempo. Sul telefono la
    ...
    const loop=()=>{
      ...
      /* [7.773.0] l'avanzata dell'avversario prima del tiro sul gol subito: finche' l'arco e' in attesa (ballArcT<0) il portatore
         corre verso le 19 unita' con il pallone ai piedi; al via del tiro si arma il tuffo del portiere e la mano si spegne. */
      if(sr.current._adv773&&ballArcActive&&isResult){const _a=sr.current._adv773;
        ...
        else if(!_a.done){_a.done=true;sr.current._advLog773=Object.assign({},sr.current._advLog773||{},{fine:true});if(_a.gk&&!oppActType){oppActType="gk_dive";{try{if(typeof window!=='undefined'&&window.__CPM_REC){const _G=(window.__CPM_GK799=window.__CPM_GK799||[]);if(_G.length<200)_G.push({tag:"T1",t:(typeof ballArcT!=="undefined"&&ballArcT!=null)?+(+ballArcT).toFixed(2):null,act:(typeof ballArcActive!=="undefined"&&ballArcActive)?1:0,dur:(typeof ballArcDur!=="undefined"&&ballArcDur!=null)?+(+ballArcDur).toFixed(2):null});}}catch(_e799){}}oppActT=0;oppMesh=_a.gk;oppDiveDir=_a.gz>=_a.gk.position.z?1:-1;_a.gk._divePz=_a.gk.position.z;_a.gk._diveYaw=_a.gk.rotation.y;_a.gk._diveToZ=clamp(_a.gz,_a.gk.position.z-6,_a.gk.position.z+6);}sr.current._adv773=null;}}
      ...
      if(ballArcActive){const _preT74=ballArcT;
        ...
        if(u>=1||_early74){
          ...
          if(isResult){const _cc460=_no460?null:sr.current._ccx460;const P=(_cc460&&(propsRef.current.hlType==null||propsRef.current.hlSuccess==null))?_cc460:propsRef.current;/* [7.460.0] SI LEGGE IL CONTESTO DICHIARATO AL LANCIO, non quello del fotogramma d'atterraggio — ma SOLO quando i props vivi hanno gia' perso la conclusione: sul percorso sano questa riga vale `propsRef.current` ed e' bit-identica a prima. */const _hs=P.hlSuccess,_ht=P.hlType,_hv=P.hlVariant;
            ...
            /* [7.772.0 collaudo PO 04/09 07:28 «gol subito mostrato come azione pericolosa ma con tiro da distanza assurda ed
               il pallone credo non sia manco entrato» — rosso __CPM_NO772] IL GOL SUBITO FINIVA IN «DEFLECT». Tracciato su 10
               scene difensive con esito gol subito (GLB ON, sonda golsub3/4-772): a fine arco il ramo hlDef qui sopra dichiara
               `in_net_own` (toOwnGoal vero, def vero), ma QUESTO blocco — il post-arco dei fallimenti, nato per i tiri
               dell'eroe (parata/palo/fuori dal tipo del motore) — gira anche sui fallimenti DIFENSIVI e lo sovrascrive con
               `deflect`: il pallone si parcheggia a gx 1,8-2,1, cioe' davanti alla linea (1,4), e in rete non entra mai
               (canale in_net_own acceso 0 volte su 10, ingresso 0 su 10). Il tabellone dice gol, il campo dice respinta.
               Sul gol subito il post-arco dei fallimenti non ha voce: comanda il canale della rete. */
            if(_hs===false&&!(P.hlDef&&P.hlDefTraj&&P.hlDefTraj.toOwnGoal&&hlPostArcType==="in_net_own"&&!(typeof window!=='undefined'&&window.__CPM_NO772))){hlInNetFlashed=false;heroPostT=0;heroPostType="miss";
        ...
        if(hlPostArcType==="in_net"){
          ...
          /* ⚠️ [7.800.0 — IL PORTIERE NON HA TEMPO DI REAZIONE: si tuffa nell'istante in cui il pallone
             parte. Diagnosi MISURATA con un metro che regge; il rimedio e' il passo dopo.]
             Nota PO (7.787, SIT #153 «Bordata da centrocampo» → goal): «codice 111 — portiere fuori
             tempo».
             ⚠️ PRIMA DI TUTTO, UNA RITRATTAZIONE. Nel 7.799 avevo scritto che il portiere «si tuffa
             quando il pallone e' gia' arrivato», con un rapporto istante/durata di 1,03. ERA UN
             ARTEFATTO DELLA MIA SONDA: registravo il tempo dell'arco senza chiedere se un arco fosse
             davvero IN CORSO, e un tuffo armato ad arco gia' finito si portava dietro la DURATA VECCHIA
             con il tempo azzerato — quindi finiva nel mucchio come se fosse partito alla fine. Chiesto
             `ballArcActive`, quei campioni spariscono e il quadro si ribalta.
             LA MISURA VERA, quattro partite intere, 39 tuffi con un arco effettivamente in corso:
             l'istante del tuffo e' ZERO in tutti e trentanove — mediana, p25, p75 e massimo. Il portiere
             parte NELLO STESSO ISTANTE in cui il pallone parte, senza un decimo di reazione. Su un tiro
             corto quasi non si nota; su una bordata da centrocampo si butta subito e poi resta a terra
             mentre la palla vola per mezzo secondo — ed e' quello che il PO ha visto e chiamato «fuori
             tempo». Non e' in ritardo: e' in ANTICIPO, di tutto il tempo di volo.
             IL METRO ORA REGGE, e questa e' la seconda correzione: giudicavo la sonda dal CONTEGGIO dei
             tuffi, che oscilla per forza (10-25 a partita) perche' dipende da quanti tiri ci sono. Il
             metro giusto e' la QUOTA, che e' una frazione: misurata sui quattro mondi ha spread ZERO
             punti. Con questo si puo' giudicare un rimedio; col conteggio no.
             IL RIMEDIO, per chi riprende: serve un TEMPO DI REAZIONE (due-tre decimi) fra la partenza del
             pallone e l'inizio del tuffo, e va messo dove il tuffo NASCE — i tuffi arrivano da quattro
             rami diversi (T2, T7, T8, T9 nel testimone), quindi non basta toccarne uno solo: e' l'errore
             che ho gia' fatto due volte su questo punto. Testimone `__CPM_GK799` su tutti e nove i siti,
             sonda `tests/visual/portiere2-800.mjs` (partite intere, metro a frazione). */
          if(!(typeof window!=='undefined'&&window.__CPM_NO652)&&awayGkMesh&&!P.hlDef&&!oppActType&&ball.position.x>GOAL_LINE_X-14){
            oppActType="gk_dive";{try{if(typeof window!=='undefined'&&window.__CPM_REC){const _G=(window.__CPM_GK799=window.__CPM_GK799||[]);if(_G.length<200)_G.push({tag:"T2",t:(typeof ballArcT!=="undefined"&&ballArcT!=null)?+(+ballArcT).toFixed(2):null,act:(typeof ballArcActive!=="undefined"&&ballArcActive)?1:0,dur:(typeof ballArcDur!=="undefined"&&ballArcDur!=null)?+(+ballArcDur).toFixed(2):null});}}catch(_e799){}}oppActT=0;oppMesh=awayGkMesh;oppDiveDir=ball.position.z>=oppMesh.position.z?1:-1;oppMesh._divePz=oppMesh.position.z;oppMesh._diveYaw=oppMesh.rotation.y;oppMesh._diveToZ=clamp(oppMesh.position.z+(ball.position.z-oppMesh.position.z)*0.55,oppMesh.position.z-6,oppMesh.position.z+6);
          ...
          if(_uh74>=0.93&&sr.current._cgHold!=null&&sr.current._cgHold<=0){sr.current._cgHold=null;/* [7.252.0 gi30 «rimbalza in area senza senso»] l'incornata del terzo uomo colpisce la palla IN ARIA (~93% dell'arco, ancora in discesa) — prima la transizione aspettava il tocco a terra e la ripartenza verso la rete leggeva come un rimbalzo */
            ...
            if(awayGkMesh&&!P.hlDef&&!oppActType){oppActType="gk_dive";{try{if(typeof window!=='undefined'&&window.__CPM_REC){const _G=(window.__CPM_GK799=window.__CPM_GK799||[]);if(_G.length<200)_G.push({tag:"T3",t:(typeof ballArcT!=="undefined"&&ballArcT!=null)?+(+ballArcT).toFixed(2):null,act:(typeof ballArcActive!=="undefined"&&ballArcActive)?1:0,dur:(typeof ballArcDur!=="undefined"&&ballArcDur!=null)?+(+ballArcDur).toFixed(2):null});}}catch(_e799){}}oppActT=0;oppMesh=awayGkMesh;oppDiveDir=ball.position.z>=oppMesh.position.z?1:-1;oppMesh._divePz=oppMesh.position.z;oppMesh._diveYaw=oppMesh.rotation.y;oppMesh._diveToZ=clamp(oppMesh.position.z+(ball.position.z-oppMesh.position.z)*0.55,oppMesh.position.z-6,oppMesh.position.z+6);}/* [6.74.0 3D-3] anche sull'incornata da cross il tuffo resta corto (il gol sta entrando) */
...
const _mx47=clamp(Math.max(Math.min(_rm.position.x+_lead54,AWAY_GOAL_X-13),ball.position.x-2,(sr.current._arLandX!=null?sr.current._arLandX-1.5:-99)),-48,GOAL_LINE_X-4);/* [7.310.0] pavimento al punto d'atterraggio: la palla non arretra a cercare il ricevente */
          ...
          if(_isCh93&&_u3>=0.72){
            if(awayGkMesh&&!oppActType){oppActType="gk_dive";{try{if(typeof window!=='undefined'&&window.__CPM_REC){const _G=(window.__CPM_GK799=window.__CPM_GK799||[]);if(_G.length<200)_G.push({tag:"T4",t:(typeof ballArcT!=="undefined"&&ballArcT!=null)?+(+ballArcT).toFixed(2):null,act:(typeof ballArcActive!=="undefined"&&ballArcActive)?1:0,dur:(typeof ballArcDur!=="undefined"&&ballArcDur!=null)?+(+ballArcDur).toFixed(2):null});}}catch(_e799){}}oppActT=0;oppMesh=awayGkMesh;oppDiveDir=ball.position.z>=oppMesh.position.z?1:-1;oppMesh._divePz=oppMesh.position.z;oppMesh._diveYaw=oppMesh.rotation.y;oppMesh._diveToZ=clamp(oppMesh.position.z+(ball.position.z-oppMesh.position.z)*0.55,oppMesh.position.z-6,oppMesh.position.z+6);}
            ...
            if(_u3>=0.55&&awayGkMesh&&!oppActType){oppActType="gk_dive";{try{if(typeof window!=='undefined'&&window.__CPM_REC){const _G=(window.__CPM_GK799=window.__CPM_GK799||[]);if(_G.length<200)_G.push({tag:"T5",t:(typeof ballArcT!=="undefined"&&ballArcT!=null)?+(+ballArcT).toFixed(2):null,act:(typeof ballArcActive!=="undefined"&&ballArcActive)?1:0,dur:(typeof ballArcDur!=="undefined"&&ballArcDur!=null)?+(+ballArcDur).toFixed(2):null});}}catch(_e799){}}oppActT=0;oppMesh=awayGkMesh;oppDiveDir=ball.position.z>=oppMesh.position.z?1:-1;oppMesh._divePz=oppMesh.position.z;oppMesh._diveYaw=oppMesh.rotation.y;oppMesh._diveToZ=clamp(oppMesh.position.z+(ball.position.z-oppMesh.position.z)*0.3,oppMesh.position.z-4,oppMesh.position.z+4);}
      ...
      if(_curPh!==prevPhase){
        if(_curPh==="hl_result"){fireConclusion=()=>{const P=propsRef.current;sr.current._ccx460=P;/* [7.460.0] IL CONTESTO DELLA CONCLUSIONE SI DICHIARA QUI, AL LANCIO — stessa cura dello `stageStamp` del 7.456: il blocco di completamento dell'arco (r.~12420) lo leggeva dal fotogramma in cui la palla atterra, e su una parte dei gol quel fotogramma cade quando i props della scena sono gia' stati svuotati (misurato: atterraggio con `ht=null hs=null rew=null`, catena dei rami che non trova nulla, nessun post-arco, palla ferma a x 42,7 mentre il tabellone conta il gol). */if(typeof window!=='undefined'&&(_CPM_TEST||_SIT_TEST)){try{const _ac=(window.__CPM_ARCCALL457=window.__CPM_ARCCALL457||[]);if(_ac.length<40)_ac.push({ph:P.matchPhase||null,ht:P.hlType||null,hs:P.hlSuccess,rew:P.hlReward||null,pat:P.hlPattern||null});}catch(_e){}}let t=P.hlType;/* [7.346.0] `let`: il ramo d'ARCO si dirama su `t`, non su `actType` — per questo il precedente su `build` correggeva il gesto ma lasciava la traiettoria in porta. La rimappatura dell'assist (poche righe sotto) agisce su ENTRAMBI. L'estrattore analitico del check data-coherence passa `t` come parametro alla sua IIFE e parte dal marcatore `const _rz=` che sta a valle: la sua valutazione resta indipendente da questa riassegnazione. */actType=(t==="header"||t==="tackle"||t==="dribble"||t==="cross"||t==="penalty"||t==="freekick"||t==="pass"||t==="build")?t:(!t)?"build":"shot";actT=0;sr.current._actSitKey403=P.hlSitKey;/* [7.403.0] l'azione appartiene a QUESTA scena: la mappa gesti non monta niente finche' la chiave non coincide */// CINE-VAR: build ora ha animazione propria; null type → "build" (gesto neutro)
          ...
          if(t==="penalty"){// Panenka/cucchiaio → arco molto alto; rigore normale → arco standard
            ...
            /* [7.788.0 — REVOCATO QUI, e a verbale perche' e' la terza volta] Avevo agganciato il
               bersaglio del cross al compagno piu' vicino al punto geometrico. MISURA APPAIATA: 12 cross
               su 24 senza nessuno entro 3,5u contro i 10 di prima, p90 14,9u contro 9,1 — PEGGIO. E il
               contatore ha detto perche': il blocco d'arco qui sotto e' stato eseguito UNA volta su otto
               scene di cross forzate. Con l'esecutore cinematico acceso (il regime del gioco vero) e' la
               TIMELINE a muovere il pallone lungo il cross, e questo arco quasi non gira: stavo curando
               un ramo che il PO non vede quasi mai, esattamente come nei due rimedi gia' revocati sul
               codice 011. Il bersaglio del cross va corretto dove nasce, in buildHLTimeline. */
          ...
          if(P.hlDef&&ballArcActive){const _px=P.playerX||50,_py=P.playerY||50;
            ...
            if(_dt.gkClaim){/* [7.238.0 gi44] «Chiama il portiere» riuscito: la palla va NELLE MANI del portiere
              ...
              /* [7.773.0 collaudo PO 04/09 07:28 «tiro da distanza assurda» — rosso __CPM_NO773] L'AVVERSARIO AVANZA PRIMA DI
                 TIRARE. Misurato su 10 scene difensive con gol subito: il tiro partiva da 13-38 unita' dalla porta (mediana ~24,
                 gi33 fino a 50), cioe' da dove stava il pallone quando la scena si e' aperta, con un arco di 0,55 s: una bordata
                 da meta' campo a 60 u/s. Qui, se il pallone e' oltre le 20 unita', il portatore avversario piu' vicino lo porta
                 fino a 18-20 unita' (8 u/s, palla ai piedi) e SOLO POI parte il tiro; il tuffo del portiere si arma al tiro,
                 non all'apertura. Sotto le 20 unita' non cambia niente. */
              const _bxg773=ball.position.x+50;const _far773=_bxg773>20&&!(typeof window!=='undefined'&&window.__CPM_NO773);
              if(_far773){let _bm773=null,_bd773=1e9;sr.current.players.forEach((pp,ii)=>{const src=(P.allPlayers||[])[ii];if(src&&src.team==='away'&&!src.gk){const d=Math.hypot(pp.mesh.position.x-ball.position.x,pp.mesh.position.z-ball.position.z);if(d<_bd773){_bd773=d;_bm773=pp.mesh;}}});
                ballArcT=-0.05;/* l'attesa e' dinamica: la mano per-fotogramma la tiene negativa finche' il portatore non e' a 19 unita' */
                sr.current._adv773={mesh:_bm773,tx:G2X(19),tz:clamp(ball.position.z,-14,14),gk:_hgk83,gz:G2Z(_gy),done:false};}
              else if(_hgk83&&!oppActType){oppActType="gk_dive";{try{if(typeof window!=='undefined'&&window.__CPM_REC){const _G=(window.__CPM_GK799=window.__CPM_GK799||[]);if(_G.length<200)_G.push({tag:"T6",t:(typeof ballArcT!=="undefined"&&ballArcT!=null)?+(+ballArcT).toFixed(2):null,act:(typeof ballArcActive!=="undefined"&&ballArcActive)?1:0,dur:(typeof ballArcDur!=="undefined"&&ballArcDur!=null)?+(+ballArcDur).toFixed(2):null});}}catch(_e799){}}oppActT=0;oppMesh=_hgk83;const _gz83=G2Z(_gy);oppDiveDir=_gz83>=_hgk83.position.z?1:-1;_hgk83._divePz=_hgk83.position.z;_hgk83._diveYaw=_hgk83.rotation.y;_hgk83._diveToZ=clamp(_gz83,_hgk83.position.z-6,_hgk83.position.z+6);}
      ...
      if(isWalk){
        ...
        if(subEntryT>=0){
          ...
          if(typeof window!=='undefined'&&window.__CPM_HERODRV769ON){try{window.__CPM_HERODRV769={hxg:+(+_hxg).toFixed(1),hyg:+(+_hyg).toFixed(1),tl:!!(tlOn&&tlMap&&tlMap.HERO===hero),cel:celebT>=0,cap:+_cap58.toFixed(1),aDt:+aDt.toFixed(3),ak:+(+ak).toFixed(3),ph:_curPh,mx:+hero.position.x.toFixed(1),mz:+hero.position.z.toFixed(1),n:(window.__CPM_HERODRV769?window.__CPM_HERODRV769.n:0)+1};}catch(_e769){}}/* [7.769 strumentazione test-only] il driver dell'eroe in scena: bersaglio, chi lo blocca, quanto avanza */
          ...
          if(bi>=tlBuildN){const last=tlSeg[tlBuildN-1];
            ...
            {const _nx395=tlSeg[bi+1];
             if(_nx395&&_nx395.kind==='carry'&&(sg.kind==='pass'||sg.kind==='through'||sg.kind==='give'||sg.kind==='cross')){
               ...
               /* [7.788.0 — SECONDO RIMEDIO AL CROSS, REVOCATO ANCH'ESSO. A verbale perche' la lezione
                  qui non e' sul calcio, e' sul metro.]
                  Avevo esteso al `cross` la rimappatura del ricevente che il 7.414 aveva scritto per il
                  `through`: il rendez-vous del 7.395 piega il bersaglio del volo verso il compagno
                  MAPPATO, e mapCineActors assegna MATE1 al compagno piu' vicino all'EROE — che su un
                  cross sta sulla fascia, non in area. Il ragionamento regge ancora, e il precedente pure.
                  QUELLO CHE NON REGGE E' LA PROVA. Quattro passate sulla stessa domanda, stesso codice:
                  10, 12, 9 e 6 cross su 24 «senza nessuno entro 3,5u». La sonda non era ripetibile
                  perche' ogni scena forzata partiva dallo stato lasciato dalla precedente; aggiunto
                  `__CPM_RESEED(gi)` come fa il gate, e i QUANTILI si sono stabilizzati (mediana 1,9-2,0,
                  massimo 14,4-14,5 su passate ripetute) mentre il CONTEGGIO sopra soglia ha continuato a
                  ballare fra 6 e 11 — perche' mezze scene stanno proprio intorno a 3,5u e saltano da una
                  parte all'altra. Il conteggio era la metrica sbagliata.
                  Confronto onesto, due passate per lato con la sonda aggiustata:
                    verde  mediana 1,9 e 2,0 · p90 7,5 e 8,2 · max 14,5 e 14,4
                    rosso  mediana 1,7 e 1,9 · p90 8,4 e 8,6 · max 14,5 e 14,4
                  Si sovrappongono. Il rimedio non batte la misura, quindi non parte — anche se
                  l'ipotesi mi convince: convincere non e' misurare. Il difetto resta APERTO e
                  documentato; per deciderlo serve un metro che risolva differenze di questa taglia. */
        ...
        if(!(typeof window!=='undefined'&&window.__CPM_NO695)&&P.gkSave&&P.gkSave.current&&P.gkSave.current.t&&P.gkSave.current.t!==_lastGk695&&!oppActType&&sr.current&&sr.current.players){
          ...
          try{const _tm695=(P.gkSave.current.side==='home')?'away':'home';let _gk695=null;
            ...
            if(_gk695){/* [7.705.0 — IL TUFFO VA DOVE VA IL PALLONE VERO, E QUANTO UN CORPO ARRIVA. Rosso __CPM_NO705]
              ...
              oppActType="gk_dive";{try{if(typeof window!=='undefined'&&window.__CPM_REC){const _G=(window.__CPM_GK799=window.__CPM_GK799||[]);if(_G.length<200)_G.push({tag:"T9",t:(typeof ballArcT!=="undefined"&&ballArcT!=null)?+(+ballArcT).toFixed(2):null,act:(typeof ballArcActive!=="undefined"&&ballArcActive)?1:0,dur:(typeof ballArcDur!=="undefined"&&ballArcDur!=null)?+(+ballArcDur).toFixed(2):null});}}catch(_e799){}}oppActT=0;oppMesh=_gk695;oppDiveDir=_bz705>=_gk695.position.z?1:-1;
      ...
      if(_curPh==='playing'&&!(typeof window!=='undefined'&&window.__CPM_NO526)&&!P.ceremony&&!P.shootout&&!replaying){
        ...
        /* ⚠️ [7.795.0 — REVOCATO. L'ipotesi era ragionevole e la misura l'ha bocciata. Rosso storico
           __CPM_NO795.]
           Nota PO sulla 7.792: «ha mostrato un ping pong ripetuto ... non e' calcio ma una sequenza di
           bug». IL DIFETTO E' RIPRODOTTO, e questa parte resta: nei cinque secondi dopo una parata il
           pallone RESO inverte il verso tre o piu' volte con ampiezze da oltre due unita' in CINQUE
           episodi su dieci (mediana 3, massimo 4). Le ampiezze — 2,0 · 2,4 · 3,1 · 4,6 · 5,5 — sono la
           distanza fra due corpi vicini.
           PER ARRIVARCI HO SBAGLIATO BERSAGLIO DUE VOLTE, ed e' la parte che vale: cercando dentro le
           scene dell'eroe, zero su otto; cercando nel flusso ma sul punto-palla LOGICO, zero su otto —
           inutile, perche' il PO guarda il pallone RESO e il codice 011 ha gia' misurato che i due
           divergono di 9-17 unita'. Uno zero sul pallone sbagliato non e' un verde.
           L'IPOTESI: il pallone reso e' incollato al portatore eletto (`ball.position = portatore + 0,9`)
           e l'elezione sceglie per vicinanza al punto LOGICO, che si muove a ogni tick; quando il
           vincitore cambia, il pallone salta da un corpo all'altro. Dopo una parata il pallone e' MORTO
           per regolamento (il 7.702 lo dichiara gia'), e un pallone morto non ha padrone: ho sospeso
           l'elezione finche' il fischio dura.
           LA MISURA L'HA BOCCIATO: verde 4 episodi su 8 (mediana 3), rosso 3 su 8 (mediana 2) — nessun
           miglioramento, semmai il contrario, e comunque dentro il rumore. La sospensione non basta,
           quasi certamente perche' il pallone resta morto solo per i quattro tick della transizione
           (~1,2 s) mentre il ping pong si distende su cinque secondi: quando l'elezione riparte, riparte
           anche il salto. Il rimedio vero e' l'anagrafe del possesso — il redesign A2 che il 7.617 ha
           gia' messo a verbale — non una porta chiusa per un secondo.
           RESTA LO STRUMENTO, che e' la cosa di valore: `tests/visual/pingpong2-795.mjs` riproduce la
           nota del PO in modo ripetibile, sul pallone giusto, con un verdetto in inversioni. */
      ...
      if(isWalk){
        ...
        if((isHL||P.matchPhase==="playing")&&!P.ceremony&&!P.shootout&&!replaying&&!(typeof window!=='undefined'&&window.__CPM_NO650)){/* [7.650.0 v2] ANCHE LA CRONACA: la broadcast dietro-porta con rotazione al 2o tempo (7.561) era esattamente cio che la direttiva vieta — misurato: la camera partiva dal lato playing e ATTRAVERSAVA il campo in transito a ogni apertura di scena (31 percento dei campioni hl sotto z16, z fino a -52,5) */
          ...
          if(isHL){
            ...
            if(!(typeof window!=='undefined'&&window.__CPM_NO766)){const _bz766=ball?ball.position.z:0,_bx766=ball?ball.position.x:tLx;if(_bz766>14){tLz=Math.max(tLz,_bz766*0.30+(_bz766-14)*1.0);}/* [7.767.0] 0,85→1,0: tracciato col hook __CPM_CAMT767 (seme 764, 44'-46'): bersaglio raggiunto (look z 25,6) e pallone a ndc y -1,06, un soffio sotto il bordo — il conto a tavolino con look y 1,4 dava 18 gradi sotto il centro, il quadro reale ne regge meno */
              ...
              const _dx766=_bx766-tLx,_mx767=clamp(0.14*(38-_bz766),1.5,6);/* [7.767.0] il margine in x segue la DISTANZA: a fov 46 il semiquadro orizzontale vale 0,19 per unita' di distanza dalla camera (z 38) — sull'angolo vicino (z 32,6) sono 1 unita', non 6: misurato ndc x 1,29 col margine fisso */if(Math.abs(_dx766)>_mx767){tLx+=_dx766-Math.sign(_dx766)*_mx767;tPx=clamp(tLx*0.85,-36,36);}}
          }
        }
        /* [7.398.0 collaudo PO #33 «si vede la curva da dietro in primo piano»] LA CAMERA RESTA NELLO
      ...
      if(typeof window!=='undefined'&&window.__CPM_CAMT767ON){try{window.__CPM_PROJ767=(x,y,z)=>{const v=new THREE.Vector3(x,y,z).project(camera);return{x:+v.x.toFixed(3),y:+v.y.toFixed(3),z:+v.z.toFixed(3)};};window.__CPM_CAMT767={tPx:+tPx.toFixed(1),tPy:+tPy.toFixed(1),tPz:+tPz.toFixed(1),tLx:+tLx.toFixed(1),tLy:+tLy.toFixed(1),tLz:+tLz.toFixed(1),kp:+kp.toFixed(4),kl:+kl.toFixed(4),fov:+(camera.fov||0).toFixed(1),cx:+camera.position.x.toFixed(1),cy:+camera.position.y.toFixed(1),cz:+camera.position.z.toFixed(1),lx:+camLook.x.toFixed(1),ly:+camLook.y.toFixed(1),lz:+camLook.z.toFixed(1),sal:!!P.salienteOn,hl:+(camHL||0).toFixed(2),isHL:!!isHL,ph:P.matchPhase||null,tocc:(sr.current._tocc503||[]).slice(-6)};}catch(_e767){}}/* [7.767 strumentazione test-only] i bersagli della regia PRIMA del liscio: senza, «lo sguardo e' dieci unita' lontano dal pallone» non distingue un lerp lento da un bersaglio sbagliato */
      ...
      if(glbAvatars){
        ...
        for(let _ai=0;_ai<glbAvatars.length;_ai++){const _a=glbAvatars[_ai],_p=_a.proc,_gdx=_p.position.x-_a.lx,_gdz=_p.position.z-_a.lz,_gsp=Math.hypot(_gdx,_gdz);
          ...
          if(_ent516){// EROE, PORTIERE o COMPAGNO che conclude (o in RILASCIO): gesto one-shot in crossfade con la locomozione
            ...
            /* [7.709.0 — LA CLIP DEL TUFFO SUONA SUL VOLO, NON SUL SUO METRONOMO. Rosso __CPM_NO709]
               Codice 111 del PO sulla 7.708 (x2, GLB). MISURATO (portiere-709, GKTL col clock della clip,
               scena della nota gi55 incornata-gol): la clip parte ~0,3s PRIMA dell'arco (12 fotogrammi di
               avanzamento col pallone fermo al wind-up) e corre piu' veloce del volo (37% clip vs 29% arco
               a t+0,66s) — proiettata, finisce con ~1/4 del volo in aria e clampWhenFinished congela il
               portiere disteso ad aspettare. Il 7.512 aveva dato alla clip l'orologio giusto in SCALA
               (_diveDur) ma non in FASE: la vita del gesto si allunga con l'arco (7.465) e la clip no.
               Qui la clip diventa uno scrub sull'estensione arco-sincrona (oppMesh._ext465): la stessa
               fonte che guida la traslazione procedurale — un orologio solo per testo, corpo e clip.
               Perimetro: solo il TUFFO del portiere che sta reagendo; catch/block restano com'erano. */
            if(_a._isGk&&_a.proc===oppMesh&&_a._gName==='dive'&&_a._gAct&&oppMesh&&oppMesh._ext465!=null&&!(typeof window!=='undefined'&&window.__CPM_NO709)){
              const _ld9=(_a._gAct.getClip()||{}).duration||1;_a._gAct.paused=true;_a._gAct.time=Math.min(_ld9*0.98,_ld9*Math.max(0,oppMesh._ext465));}
      ...
      /* [testimone 7.803 — CHI VINCE SUL PALLONE. Sola lettura sotto __CPM_REC]
         Il pallone reso ha QUINDICI scrittori in questo file e uno solo segue il pallone logico
         (`P.ballX`, cioe' cio' che telecronaca, piani e gol affermano). Misurato: al gol il logico
         entra in rete e ci resta 900 ms, il reso non si muove. Qui, a fine fotogramma, si registra
         di quanto il reso si discosta dal logico e QUALI sistemi erano attivi — cosi' il rimedio
         sa quale dei quattordici togliere di mezzo, invece di spegnerli tutti alla cieca. */
      if(typeof window!=='undefined'&&window.__CPM_REC){try{
        const _P=propsRef.current||{};
        if(_P.ballX!=null){
          const _atteso=G2X(_P.ballX), _vero=ball.position.x, _d=Math.abs(_vero-_atteso);
          const _L=(window.__CPM_PADRONE=window.__CPM_PADRONE||{n:0,fuori:0,somma:0,max:0,chi:{}});
          _L.n++;_L.somma+=_d;if(_d>_L.max)_L.max=_d;
          if(_d>2){_L.fuori++;
            /* ⚠️ la prima stesura inventava un campo `_carrier` che NON ESISTE, e dava 0% al
               portatore mettendo tutti i suoi casi nel mucchio «altro»: uno zero netto su un
               sistema che si sa attivo e' sempre lo strumento, non il gioco. Il nome vero
               ce l'ha gia' il codice: ogni scrittore firma `sr.current._bj0.src`. */
            const _chi=(sr.current&&sr.current._bj0&&sr.current._bj0.src)||(ballArcActive?'arco':'(nessuna firma)');
