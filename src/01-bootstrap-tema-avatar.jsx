...
/* [23/09 POC — FIGURINA NEL SISTEMA GRAFICO DEL GIOCO, direttive PO «il testo non deve strabordare» e «i colori del gioco
   devono essere rispettati anche dalle figurine»]
   - colori: pannello TH.card, testo TH.text/TH.muted, filetti TH.divider, ombre TH.el2/el3 (seguono il tema chiaro/scuro); la
     cornice resta nei colori del club, e senza club usa TH.primary -> TH.primaryDk; il testo sulla pastiglia del ruolo e' bianco o
     TH.text secondo il contrasto misurato (_rap944);
   - tipografia: il carattere del gioco (Barlow, ereditato), SOLO la scala FS (mai sotto FS.caption = 11 px) e i pesi FW;
     raggi dalla scala RAD;
   - NIENTE ELLISSI: ogni riga sceglie il passo FS piu' grande che entra nella larghezza; se nemmeno FS.caption entra, il ruolo
     perde la coda dopo « · », il cognome va su due righe; le righe che non entrano in ALTEZZA non si disegnano (priorita':
     cognome, ruolo, altezza/peso, nome). Rosso __CPM_NO_CORNICE23B (cornice del primo giro). */
...
function FigurinaKorward23({url,tipo,nome,ruolo,col,col2,w,h,titolo,style,rest,chiave}){
  ...
  const pad=w>=140?8:5,disp=lato-2*pad;
  const righe=[];
  if(w>=52&&!vecchia){
    const cogn=String(D.cognome||"").toUpperCase();
    if(cogn){const f=_passoFS23(cogn,disp,[FS.subhead,FS.bodyLg,FS.body,FS.small,FS.caption],FW.black);righe.push({k:"cognome",t:cogn,fs:f||FS.caption,a:f?1:Math.ceil(_largTesto23(cogn,FS.caption,FW.black)/Math.max(1,disp))});}
    if(D.ruolo){let r=String(D.ruolo).toUpperCase();let f=_passoFS23(r,disp-12,[FS.small,FS.caption],FW.bold);if(!f&&r.indexOf(" · ")>0){r=r.split(" · ")[0];f=_passoFS23(r,disp-12,[FS.small,FS.caption],FW.bold);}if(f)righe.push({k:"ruolo",t:r,fs:f,a:1});}
    if(D.cm){const t=D.cm+" cm · "+D.kg+" kg";const f=_passoFS23(t,disp,[FS.small,FS.caption],FW.semibold);if(f)righe.push({k:"fisico",t,fs:f,a:1});}
    if(D.nome){const f=_passoFS23(D.nome,disp,[FS.body,FS.small,FS.caption],FW.medium);if(f)righe.push({k:"nome",t:D.nome,fs:f,a:1});}
  }
  ...
  return(
    <div data-cpm-figurina={tipo} data-cpm-volto="1" data-cpm-fig-piena={usate.length>=3?"1":undefined} title={titolo||[D.nome,D.cognome].filter(Boolean).join(" ")||undefined}
      style={{position:"relative",width:w,height:h,flexShrink:0,borderRadius:rag,overflow:"hidden",fontFamily:_FONT23,
      background:`linear-gradient(155deg, ${c1} 0%, color-mix(in srgb, ${c1} 62%, #000000) 60%, ${c2} 100%)`,
      boxShadow:w>=140?TH.el3:TH.el1,...style}} {...(rest||{})}>
      <div aria-hidden="true" style={{position:"absolute",inset:0,pointerEvents:"none",zIndex:2,
        background:"linear-gradient(115deg, rgba(255,255,255,0) 35%, rgba(255,255,255,0.18) 47%, rgba(255,255,255,0.05) 53%, rgba(255,255,255,0) 62%)"}}/>
      {marchio&&<div style={{height:hTop,display:"flex",alignItems:"center",justifyContent:"center",gap:6,color:"#ffffff",
        fontWeight:FW.bold,letterSpacing:w>=140?3:1,fontSize:hTop>=18?FS.small:FS.caption,lineHeight:1,textShadow:"0 1px 2px rgba(0,0,0,.35)",whiteSpace:"nowrap",overflow:"hidden"}}>
        {w>=140&&<span style={{width:Math.round(w*0.07),height:1,background:"rgba(255,255,255,0.7)"}}/>}{_largTesto23("KORWARD",hTop>=18?FS.small:FS.caption,FW.bold)+6*(w>=140?3:1)<=lato?"KORWARD":null}{w>=140&&<span style={{width:Math.round(w*0.07),height:1,background:"rgba(255,255,255,0.7)"}}/>}</div>}
      ...
      {usate.length>0&&<div style={{position:"absolute",left:bordo,right:bordo,bottom:bordo,height:hPan,
        ...
        {usate.map(r=>r.k==="ruolo"
          ?<div key={r.k} style={{fontSize:r.fs,lineHeight:1.18,color:inkPill,background:c1,borderRadius:RAD.pill,padding:"1px 6px",fontWeight:FW.bold,letterSpacing:.4,whiteSpace:"nowrap"}}>{r.t}</div>
          :<div key={r.k} className={r.k==="fisico"?"cpm-num":undefined} style={{fontSize:r.fs,lineHeight:1.18,maxWidth:"100%",
              color:r.k==="cognome"?TH.text:TH.muted,fontWeight:r.k==="cognome"?FW.black:(r.k==="fisico"?FW.semibold:FW.medium),
              letterSpacing:r.k==="cognome"?.4:0,whiteSpace:r.a>1?"normal":"nowrap",wordBreak:"normal",overflowWrap:r.a>1?"anywhere":"normal"}}>{r.t.replace(/-/g,"-\u200b")}</div>)}
      </div>}
    </div>);
}
