  ...
  return(
    <div style={{width:"100%"}}>
      <div style={{textAlign:"center",marginBottom:14,paddingTop:6}}>
        {(()=>{const _lg=player.club?.lg||"Primavera 1";const _isP2=_lg==="Primavera 2";return(<><div style={{fontSize:44,marginBottom:6}}>{_isP2?"🥈":"🎓"}</div><h1 style={{fontSize:19,fontWeight:900,margin:"0 0 4px",color:TH.text}}>Fine stagione {_lg}</h1><p style={{color:TH.muted,fontSize:FS.small,margin:0}}>Stagione {u18S+1} completata · {_lg}</p></>);})()}
      </div>
      {/* Primavera awards */}
      ...
      <Card style={{marginBottom:12,padding:"12px 14px"}} bg={forced?"#fff1f2":"#eff6ff"} border={forced?"#fecaca":"#bfdbfe"}>
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          ...
          <div style={{textAlign:"right",flexShrink:0}}>
            <div style={{fontSize:26,fontWeight:900,color:forced?TH.danger:TH.primary,lineHeight:1}}>{u18S+1}<span style={{fontSize:FS.caption,color:TH.faint,fontWeight:400}}>/2</span></div>
      ...
      <div style={{fontSize:FS.caption,color:TH.faint,textAlign:"center",marginTop:10}}>Scegli con cura — questa decisione definirà la tua carriera professionale.</div>
      </div>{/* end cpm-proto */}
    </div>
  );
}
...
function SeasonEndScreen({data,player,onNewSeason,onRetire,notifBusy,farewell}){/* [7.432.0] farewell = ritiro ANNUNCIATO a inizio stagione: la fine stagione onora l'annuncio */
  ...
  const renderRow=(t,i)=>(
    <div key={t.id||i} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 8px",borderRadius:RAD.xs,
  ...
  if(sorted.length>0){
    ...
    if(!playerInTop&&!playerInBot){
      rows.push(<div key="sep1" style={{textAlign:"center",fontSize:FS.caption,color:TH.faint,padding:"2px 0"}}>…</div>);
      rows.push(renderRow(sorted[playerPos],playerPos));
    }
    if(!playerInBot||(sorted.length<=6)){
      if(!playerInTop&&!playerInBot)rows.push(<div key="sep2" style={{textAlign:"center",fontSize:FS.caption,color:TH.faint,padding:"2px 0"}}>…</div>);
      botN.forEach((t,i)=>{ const ri=sorted.length-3+i; if(ri>2&&ri!==playerPos)rows.push(renderRow(t,ri)); });
    }else if(sorted.length>3){
      rows.push(<div key="sep2" style={{textAlign:"center",fontSize:FS.caption,color:TH.faint,padding:"2px 0"}}>…</div>);
  ...
  return(
    <div style={{width:"100%"}}>
      ...
      <div style={{textAlign:"center",marginBottom:14,paddingTop:6,
        ...
        borderRadius:RAD.lg,padding:"16px 8px 12px",marginTop:4}}>
      ...
      {sorted.length>0&&(
        <Card style={{marginBottom:12,padding:"12px 14px"}}>
          ...
          {rows}
          <div style={{display:"flex",gap:10,marginTop:8,fontSize:FS.caption,flexWrap:"wrap"}}>
      ...
      {sorted.length>0&&!_isYouth99&&(
        <Card style={{marginBottom:12,padding:"12px 14px"}}>
          {_isTopFlight132?(
            <>
              <div style={{fontSize:FS.caption,color:TH.muted,textTransform:"uppercase",letterSpacing:1.5,marginBottom:8}}>🌍 Qualificazioni Europee</div>
