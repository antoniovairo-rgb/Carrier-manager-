  ...
  React.useEffect(()=>{
    ...
    const tick=()=>{
      ...
      const _posAt=(t)=>{/* → [hx,hz,run,armsUp] · [7.32.5 collaudo PO «manovra non fluida»] la corsa è
        ...
        const p=Math.min((t-9.6)/2.4,1),s=p*p*(3-2*p);return[5.4*s,-4.9-((typeof window!=='undefined'&&window.__CPM_NO_LED23)?8.5:7.3)*s,p<0.9?1:0,p>0.8];};/* [23/09 POC — collaudo PO «il piede va nel cartellone»] l'arrivo era a z -13,4, DIETRO il LED (z -13,1, x 4,1..10,3): gambe coperte. Ora z -12,2, 0,9 davanti. Rosso __CPM_NO_LED23 */
  ...
  for(let s=1;s<=50;s++){
    ...
    for(let w=1;w<=38;w++){
      ...
      if(md){
        ...
        const sim=simulateMatch(p.club,opp,p.ovr,md.isHome,(_tSd+1)>>>0,p.clubPrestigeShifts||{},{motore:true,stile:(p.coach&&p.coach.style)||null});
