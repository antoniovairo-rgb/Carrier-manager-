...
function LiveMatch({player,opponent,context="career",onMatchEnd,isMatchHome=true,benchStart,benchReason="",entryMinute=60,titleStakes=null,mdEuroPhase=null,onQuit=null,onSimulateNat=null,resumeState=null}){/* [7.150.0] resumeState: ripresa DENTRO la partita dopo background (clock/punteggio salvati → rientra in fase playing) *//* [7.14.0] onSimulateNat: Simula dal pre-partita per le gare di Nazionale (entrate dal CTA, senza altra uscita) *//* [6.77.0] onQuit: uscita pulita dal matchday (pre-partita, nessuno stato toccato) *//* [6.74.0 QA-28] mdEuroPhase: fase KO della voce calendario → il banner rigori usa lo STESSO seed della risoluzione */
  ...
  const addCom=useCallback((text,color,t)=>{lastComWallRef.current=Date.now();setComs(p=>[{text,color,t:t??clockRef.current},...p].slice(0,16));},[]); // stable — clockRef via ref, non dipende da clock state
  ...
  useEffect(()=>{
    ...
    const iv=setInterval(()=>{
      ...
      setClock(c=>{
        ...
        if(nx>=90){cpmEv("fine",{min:90,causa:"clock"});/* [7.500.0 F4] */if(!_SIT_TEST)_goEndOrCeremony();return 90;}/* [7.2.0] titolo vinto → premiazione 3D, altrimenti "ended" */
        ...
        /* ⚠️ RI-TARATO DOPO IL GUARDIANO, non contro di lui. La prima stesura forzava la riga per TUTTA
           ...
           resta. */
        const _forza541=(_cat559&&!!pendingGoalRef.current&&!_inHL77&&_advT501>=45);/* [7.542.0 v3] LA FINESTRA DEL RACCONTO COMINCIA A META' CAMPO, NON AL LIMITE DELL'AREA. La v2 apriva la forzatura solo fuori dalla costruzione, cioe' da avanzamento 58: ma il gol si scrive a 72, quindi l'attacco aveva solo la striscia 58-72 per farsi raccontare — due o tre tick, proprio sul filo della soglia «tre righe nei quattro minuti prima», e la misura infatti restava a 4/7. Ora la finestra si apre quando l'azione supera la meta' campo (45), che e' anche quando una telecronaca vera alza la voce. Resta fuori il palleggio nella propria meta', che e' il respiro che bg-rhythm difende. */
        if(_simEv77||(_draw541&&!_inHL77&&!_pausa485)||_forza541){/* [7.528.0 v2] durante l'azione pendente le righe ESCONO e raccontano l'avanzata (la decisione F3b segue la palla che sale: sviluppo/pericolo emergono da soli — la prima stesura le sopprimeva e il guardiano bg-rhythm e' diventato CIECO: sviluppo 6 coppie, pericolo 2, contro 14/13 storici); a non muovere il pallone ci pensa il blocco _bt498 qui sotto */
          ...
          let eligible=BG_MATCH.filter(e=>_noGoal77(e)&&(!e.minClock||nx>=e.minClock)&&(!e.maxClock||nx<=e.maxClock)&&(!e.derbyOnly||!!drby)&&(!e.ctx||e.ctx===_sctx76)&&!_lastBG.includes(e.txt)&&(!onBenchRef.current||!e.txt.includes("{P}"))&&(!e.momThreshold||(momentumRef.current>=e.momThreshold[0]&&momentumRef.current<=e.momThreshold[1])))
            ...
            .map(e=>{const _bp528=ballPosRef.current||{x:50,y:50};
...
/* [7.525.0] dopo il gol ambientale, come nel calcio vero: tick di volo in rete + respiro e si RIPARTE DAL CENTRO · [7.530.0 NO536] chi ha subito rimette in gioco */
          ...
          cpmEv("chronicle",{min:nx,dec:_dec499,fase:_fase501,tn:possTurnRef.current,rec:_recHij545?1:0,pd:ev.pd||null,concorda:(ev.pd===_dec499),ef:ev.ef||null,bx:ev.bpos?ev.bpos.x:null,by:ev.bpos?ev.bpos.y:null,shots:(ev.ms&&ev.ms.shots)||0,oppShots:(ev.ms&&ev.ms.oppShots)||0,sp:ev.sp||null,at:ev.at||null}/* [7.545.0] `sp` e `at` nel registro: senza, una riga che ANNUNCIA una palla ferma non e' distinguibile da una che non lo fa, e il pezzo «gli eventi fermi esistono» non ha come misurarsi */);/* [7.532.0] tn: il turno al momento della riga — strumentazione permanente per il ritmo side-aware */
          ...
          {const _ms=ev.ms||{};
           ...
           bgCoolRef.current=(typeof window!=='undefined'&&window.__CPM_NO490)?3:(_imp?7:_cool501);
