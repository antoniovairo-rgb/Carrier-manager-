...
function creaMotorePossesso(cfg){
  ...
  function decidiTenuta(){
    ...
    if(rnd()<pCond){const _T23=(press<3&&S.tab)?S.tab[P.team]:null;if(_T23)_T23.dribbling++;/* [23/09 POC] conduzione con l'avversario addosso = dribbling tentato */
      /* [24/09 POC — BRAIN «PARTITA VERA», primo difetto misurato] IL DRIBBLING NON RIESCE QUASI SEMPRE. Il dribbling
         falliva solo con spazio<2 e al 22 %: al banco (20 partite, tests/brain/pagella.mjs) 96 % di riusciti, contro il
         36,7 % di media della Premier League 2025-26 (Opta Analyst). Ora con l'avversario addosso la riuscita dipende
         dall'uomo contro uomo — dribbling di chi porta palla contro fisico/posizionamento di chi difende, piu' lo spazio
         che ha davanti — con base ~42 %. Chi perde il duello: fallo subito (30 %) o palla persa. Rosso __CPM_NO_DRIB23. */
      if(press<3&&!(typeof window!=='undefined'&&window&&window.__CPM_NO_DRIB23)){
        let _D=null,_dd=99;for(const q of g){if(!attivo(q)||q.team===P.team||q.gk)continue;const d=hyp(q.x,q.y,P.x,P.y);if(d<_dd){_dd=d;_D=q;}}
        const _a=attrsDi(P),_b=_D?attrsDi(_D):_a;
        const _pOk=clamp(0.42+((_a.dribbling+_a.velocità)/2-(_b.fisico+_b.posizionamento)/2)*0.012+(spazio>=2?0.08:-0.06)+(P.eroe?0.04:0),0.18,0.72);
        if(rnd()>=_pOk){if(rnd()<0.30){ramo("dribblingFallo");fallo(P);return;}ramo("dribblingPerso");perdi(P);return;}}
      else if(press<3&&spazio<2&&rnd()<0.22){if(rnd()<0.35){if(_T23)_T23.dribblingOk++;ramo("dribblingFallo");fallo(P);return;}ramo("dribblingPerso");perdi(P);return;}
...
function simulaPartitaMotore(o){o=o||{};
  ...
  const R=i=>i===0?'GK':i<=4?'DF':i<=7?'MF':'AT';/* [7.999.6] i ruoli del gioco: con POR/DIF/CEN/ATT il motore non riconosceva i reparti */
